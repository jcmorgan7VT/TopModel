...
sheds <- readOGR(dsn='C:/Users/John/Documents/VT Research/HBTopModel/HB/hbef_wsheds',layer = 'hbef_wsheds')
...
fit2 <- sen(n ~ year, tr)
fit <- mblm(Cells_flowing ~ numberDate, dataframe = tr)
...
counts %>% ggplot(aes(x = year, y = n))+
  ...
  labs(title = "Full network expansions by Watershed",
       x = " ",
...
ggplot() +  
  geom_raster(data=test3, aes(x=x, y=y, fill=as.factor(layer)))+
