...
network <- graph_from_data_frame(d=edges2, directed=T) 
E(network)$weight
# plot it
plot(network,layout = layout_nicely(network), edge.arrow.size = 0.25,
     vertex.size = 15,
...
is_weighted(network)
#greedy algorithm
mst <- mst(network, weights = edges2$weight)
...
tour <- solve_TSP(tsp1, method = "cheapest_insertion")
...
opt_routes <- as.data.frame(edges) %>% 
  rename("parent" = V2, "child" = V1) %>% 
 rename(new_nodes = child) %>% 
  left_join(key, by = "new_nodes") %>% 
  rename(child = old_nodes,
         done = new_nodes,
         new_nodes = parent) %>% 
  left_join(key, by = "new_nodes") %>% 
  rename(done2 = new_nodes,
         parent = old_nodes) %>% 
  select(parent, child)
...
tours <- lapply(methods, FUN = function(m) solve_TSP(tsp1, method = m))
names(tours) <- methods
```
