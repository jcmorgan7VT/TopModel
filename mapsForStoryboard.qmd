...
mod3 <- drop_na(input[,-1]) %>% select(up,down)
no_dupes <- mod3 %>%
  filter(row_number() == 1 | !apply(. == lag(.), 1, all))
#make it so that there cannot be a sequence without change
...
# successfully fixed problem- something wrong with method of subsetting dataframe
###############
...
routes <- read_csv("zz_flowrouting.csv") %>% drop_na() %>% 
  mutate(up = paste0("r_",up),
         down = paste0("r_",down))
z <- 10
input %>% select((routes$up[10]),(routes$down[10]))
...
hist(alldat$prop)
```
Test pairs for other watersheds:
```{r}
#now make loop to loop through all pairs of sensors
fb_routes <- read_csv("fb_flowrouting.csv") %>% drop_na()

for(x in 1:length(fb_routes$up)){
  up <- paste0("r_",fb_routes$up[x])
  down <- paste0("r_",fb_routes$down[x])
...
Now I can test pairs of sensors with any scheme, such as paired based on local persistency, or topographic metrics
