...
plot23 <- bind23 %>% 
  ...
  mutate(wshed = "W3") %>% 
    ggplot()+
    geom_boxplot(aes(x = ID, y = timeperiod, group = ID))
...
#plot24 <- 
  ...
  summarise(avg_days_flowing = mean(proportion)) %>% 
  mutate(wshed = "W3") %>% 
    print(n = 30)
    ggplot()+
    geom_boxplot(aes(x = ID, y = timeperiod, group = ID))
...
bind24 %>% 
  filter(ID == 1) %>% 
ggplot()+
  geom_line(aes(x = datetime, y = binary))
...
ggplot()+
  geom_tile(data = filter(bind24, wshed == "W3", mins %in% c(0, 30)),
            aes(x = datetime, y = ID, fill = as.character(binary)))+
  ...
  labs(title = "Streamflow permanence in W3, 2024",
       x = "")+
  theme_classic()

#i can either have the number of times that it changes state, easier to make a proportion

#or the average duration of flow divided by the total period length
```
```{r dof-fb}
...
sults_so_far %>% 
  filter(timescale %in% c("hourly", "daily")) %>% 
...
#Last part of analysis: try to predict the flow permanence based on topographic metrics
...
fb_locs <- read_csv("./STIC_uaa/fb2.csv") %>% 
  select(ID, POINT_X, POINT_Y)
zz_locs <- read_csv("./STIC_uaa/zz2.csv")%>% 
  select(ID, POINT_X, POINT_Y)
```
...
#extract values
vals_df <- bind_cols(extract(rast(flowacc_output), locs_shape),
          extract(rast(slope_output), locs_shape)[,2],
          extract(rast(twi_output), locs_shape)[,2],
          extract(rast(tpi_output), locs_shape)[,2]) %>% 
  rename("uaa" = '10mdem_flowacc', "slope" = `...3`, "twi" = `...4`, "tpi" = `...5`) %>% 
  left_join(pks_w3, by = "ID") %>% 
  select(-wshed, -ID)
...
#final output, make sure to add pk
W3_topo_pk <- W3_uaa_ex %>% 
  rename() %>%
  left_join(pks_w3, by = "ID")
FB_topo_pk <- FB_uaa_ex %>% 
