...
```{r looping-FB}
#make list that make pairs of sites based on local persistency
#REMOVE SITES WHERE FLOW IS 100% OF THE TIME
routes <- pks_fb %>% 
  filter(pk != 1) %>% 
  arrange(desc(pk)) %>% 
  mutate(down = lag(ID)) %>% 
...
zz_pairs_pk <- alldat
#this shows the proportion of time that pairs of upstream/downstream sensors behave according to the hypothesis of wetting up and drying down
hist(zz_pairs_pk$prop)
hist(zz_pairs$prop)
```
##Compile all of my results so far
...
w3_result_position <- data.frame("prop" = w3_pairs$prop, "type" = "position", "shed" = "W3")
w3_result_pk <- data.frame("prop" = w3_pairs_pk$prop, "type" = "pk", "shed" = "W3")

fb_result_position <- data.frame("prop" = fb_pairs$prop, "type" = "position", "shed" = "FB")
fb_result_pk <- data.frame("prop" = fb_pairs_pk$prop, "type" = "pk", "shed" = "FB")

zz_result_position <- data.frame("prop" = zz_pairs$prop, "type" = "position", "shed" = "ZZ")
zz_result_pk <- data.frame("prop" = zz_pairs_pk$prop, "type" = "pk", "shed" = "ZZ")

sults_so_far <- rbind(w3_result_position, w3_result_pk,
                      fb_result_position, fb_result_pk,
                      zz_result_position, zz_result_pk)
...
sults_so_far %>% 
  ...
  labs(x = "Watershed",
       y = "Proportion",
       title = "Proportion of times that adjacent sensors follow the hierarchy")
...
alldat$prop <- alldat$points/alldat$total
w3_pairs_topo <- alldat
#this shows the proportion of time that pairs of upstream/downstream sensors behave according to the hypothesis of wetting up and drying down
hist(w3_pairs_topo$prop)
hist(w3_pairs$prop)
...
alldat$prop <- alldat$points/alldat$total
fb_pairs_topo <- alldat
#this shows the proportion of time that pairs of upstream/downstream sensors behave according to the hypothesis of wetting up and drying down
#this shows the proportion of time that pairs of upstream/downstream sensors behave according to the hypothesis of wetting up and drying down
hist(fb_pairs_topo$prop)
hist(fb_pairs$prop)
...
alldat$prop <- alldat$points/alldat$total
zz_pairs_topo <- alldat
#this shows the proportion of time that pairs of upstream/downstream sensors behave according to the hypothesis of wetting up and drying down
hist(zz_pairs_topo$prop)
hist(zz_pairs$prop)
```
...
# I think that both position and DOF are better than PK for ZZ, meaning that relative position and average duration of flow are more likely to be followed than flow duration...
```
...
formatted_output <- data.frame("prop" = alldat$prop, "type" = hierarchy, "shed" = watershed, timing = timescale)
...
#calc_support <- function(up, down, input){
i <- 1
up <- paste0("r_",routes$up[i]) #input
