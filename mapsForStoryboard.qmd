...
#add randomly generate hierarchies
```{r define-function-and-w3}
...
random_iteration(2, "FB", "Relative Position")




#function that generated 100 random iterations
#w3_10_random <- random_iteration(sample, 100)

intermediate <- sults_so_far %>% 
  filter(shed == "W3", timescale == "30mins", hierarchy == "Duration of Flow") 
test <- intermediate$prop

w3_10_random %>% ungroup() %>% 
  filter(timescale == "30mins") %>% 
  select(prop, iteration) %>% 
  group_by(iteration) %>% 
  summarise(pval = wilcox.test(test, prop, alternative = "greater", exact = FALSE, conf.int = TRUE)$p.value) %>% 
  mutate("comparison" = "Drainage Area",
         shed = "W3") %>% summary(pval)

#testing binom test

wilcox.test(test, rand_test$prop, alternative = "greater", exact = FALSE, conf.int = TRUE)$p.value

#determine if the randomly generated scenarios are significantly different
#this function does not work
random_comparison <- function(randomdf, propdf, time){
  randomdf <- filter(randomdf, timescale == time)
    propdf <- filter(propdf, timescale == time)

  list_of_its <- unique(randomdf)
  number_of_its <- length(list_of_its)
  for(r in 1:number_of_its){
      randomdf <- filter(randomdf, iteration == r)

    pval <- wilcox.test(propdf$prop, randomdf$prop, 
                        alternative = "greater", 
                        exact = FALSE, conf.int = TRUE)$p.value
    
    out <- data.frame("iteration" = r,
                      "pval" = pval)
    
    if(r == 1) alldat <- out
    if(r > 1) alldat <- rbind(alldat, out)
  }
  return(alldat)
}
random_comparison(w3_10_random, intermediate, "30mins")
```
...
random_w3 <- w3_10_random %>% filter(iteration == 2) %>% 
  mutate(hierarchy = "Random") %>% 
  select(-iteration)
