...
sults_so_far %>% 
  filter(timescale %in% c("4hr", "daily")) %>% 
...
#Running analysis o n wetting versus drying
...
#test function
calc_support_combos("r_23", "r_20", input)
...
#calculate_hierarchy_combos(routes, input, days(1))
...
# plot it
igraph_options(label.cex = 2, arrow.size = 0.1)
plot(network)
...
####Creating a new randomly generated dataset where the flow permanence was randomly generated
```{r figuring-it-out}
p_load(fitdistrplus)
rbind(pks_w3, pks_fb, pks_zz) %>% 
  ggplot(aes(x = wshed, y = pk))+
...
```{r}
start <- ymd_hms("2024-06-09 00:00:00")
stop <- ymd_hms("2024-06-22 00:00:00")
input_w3 %>% 
  filter(datetime > start & datetime < stop) %>% #filter(ID == 17)
  #filter(threshold == "agree_or_no_90") %>% 
  ggplot()+
  geom_tile(aes(x = datetime, y = ID, fill = as.character(binary)))+
  #facet_grid(~name, scales = "free") + 
  scale_fill_manual(drop = FALSE,
                     values = binary,
                    breaks = c(0, 1),
