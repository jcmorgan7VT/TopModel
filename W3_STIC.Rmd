---
title: "W3_network_STIC"
author: "John Morgan"
date: "2023-11-02"
output: html_document
editor_options: 
  chunk_output_type: console
---
testing to make sure this works, new branch
```{r}
#setup
library(pacman)
p_load(tidyverse, sp, mapview, tmap, stars,
       lubridate, gganimate, animation, patchwork, ggrepel, whitebox,
       terra,  tidyterra)
#bring in STICr function
source("classify_wetdry.R")
source("tidy_hobo_data.R")

```

3/16/26- need a new, clean way to read in all of my data. Maybe even make a new repo with it.

folders with downloaded sensor data. Not all exist that are in spreadsheet
spreadsheet with sensor id, site id, start and end dates, sensor location

provide location to spreadsheet
provide location to folder
get list of sensors from spreadsheet
get list of sensors from folder
read sensors, and give them columns of additional info from spreadsheet
```{r read_deployment function}
read_deployment <- function(folder, study_shed, deployment_number) {
  #file location
  # folder = "./HBLoggers_11_14_23/"
  # study_shed = "W3"
  # deployment_number = 3
  # i = 1
  files <- list.files(folder, pattern = ".csv")
  #deployment info
  deploy <- read.csv("./Fieldwork_formatted_summer23/All_deployments.csv") %>% 
    # format start and end date with lubridate
    mutate(start = mdy_hm(start),
           end = mdy_hm(end),
           sensor = as.character(sensor)) %>% 
    # filter to correct watershed and deployment
    #filter values must have different names than colnames of spreadsheet
    filter(
      wshed == study_shed & deployment == deployment_number)
  # list of sensor numbers that there are actually files for
  sensor_num <- (substr(files, 1, 8))
  deployed <- deploy$sensor
  
  deployed_and_collected <- intersect(sensor_num, deployed)
  
  for (i in 1:length(deployed_and_collected)) {
  #i <- 26
    chosen <- paste0(deployed_and_collected[i], ".csv") #line to add i
    skip_to_next <- FALSE
    
    tryCatch({
      path_to_chosen <- paste0(folder, chosen)
      data <- tidy_hobo_data(path_to_chosen)
      
      #subset deployment information
      deployment_info <- dplyr::filter(deploy, sensor == deployed_and_collected[i])
      
      wet <- classify_wetdry(
        data,
        classify_var = "condUncal",
        method = "absolute",
        threshold = 10
      )
      #remove dates before and after start and end date of deployment
      wet_filt <- wet %>% 
        bind_cols(deployment_info) %>% 
        # mutate(start = deployment_info$start,
        #        end = deployment_info$end)
        dplyr::filter(datetime > start) %>%
        dplyr::filter(datetime < end) #%>%
        #bind_cols(deployment_info)
      
      
      if (i == 1)
        alldat <- wet_filt
      if (i > 1)
        alldat <- rbind(alldat, wet_filt)
    }, error = function(e) {
      skip_to_next <<- TRUE
    })
    if (skip_to_next) {
      next
    }
  }
  
  #create new column that indicates if every sensor is deployed or not
  alldat$all_deployed <- 0
  
  alldat$all_deployed[alldat$datetime > max(alldat$start) &
                        alldat$datetime <= min(alldat$end)] <- 1
  
  readout <- alldat %>% dplyr::select(-start, -end)
  
  return(readout)
}
```
```{r read_deployment function}
w3_3 <- read_deployment(folder = "./HBLoggers_11_14_23/", 
                        study_shed = "W3",
                        deployment_number = 3) %>% tibble()

read_deployment <- function(folder, study_shed, deployment_number) {
  #file location
  # folder = "./HBLoggers_11_14_23/"
  # study_shed = "W3"
  # deployment_number = 3
  # i = 1
  files <- list.files(folder, pattern = ".csv")
  #deployment info
  deploy <- read.csv("./Fieldwork_formatted_summer23/All_deployments.csv") %>% 
    # format start and end date with lubridate
    mutate(start = mdy_hm(start),
           end = mdy_hm(end),
           sensor = as.character(sensor)) %>% 
    # filter to correct watershed and deployment
    #filter values must have different names than colnames of spreadsheet
    filter(
      wshed == study_shed & deployment == deployment_number)
  # list of sensor numbers that there are actually files for
  sensor_num <- (substr(files, 1, 8))
  deployed <- deploy$sensor
  
  deployed_and_collected <- intersect(sensor_num, deployed)
  
  for (i in 1:length(deployed_and_collected)) {
  #i <- 26
    chosen <- paste0(deployed_and_collected[i], ".csv") #line to add i
    skip_to_next <- FALSE
    
  
      path_to_chosen <- paste0(folder, chosen)
      data <- tidy_hobo_data(path_to_chosen)
      
      #subset deployment information
      deployment_info <- dplyr::filter(deploy, sensor == deployed_and_collected[i])
      
      wet <- classify_wetdry(
        data,
        classify_var = "condUncal",
        method = "absolute",
        threshold = 10
      )
      #remove dates before and after start and end date of deployment
      wet_filt <- wet %>% 
        bind_cols(deployment_info) %>% 
        # mutate(start = deployment_info$start,
        #        end = deployment_info$end)
        dplyr::filter(datetime > start) %>%
        dplyr::filter(datetime < end) #%>%
        #bind_cols(deployment_info)
      
      
      if (i == 1)
        alldat <- wet_filt
      if (i > 1)
        alldat <- rbind(alldat, wet_filt)
}
  
  #create new column that indicates if every sensor is deployed or not
  alldat$all_deployed <- 0
  
  alldat$all_deployed[alldat$datetime > max(alldat$start) &
                        alldat$datetime <= min(alldat$end)] <- 1
  
  readout <- alldat %>% dplyr::select(-start, -end)
  
  return(readout)
}
```

```{r}
read_deployment24 <- function(folder, study_shed) {
  #file location
  # folder = "./summer2024/W3_summer24/"
  # study_shed = "W3"
  # i = 1
  files <- list.files(folder, pattern = ".csv")
  #deployment info
  deploy <- 
    read_csv("./summer2024/stic2024.csv") %>% 
    # format start and end date with lubridate
    mutate(start = mdy_hm(start),
           end = mdy_hm(stop),
           sensor = as.character(sensor)) %>% 
    # filter to correct watershed and deployment
    #filter values must have different names than colnames of spreadsheet
    filter(
      wshed == study_shed)
  # list of sensor numbers that there are actually files for
  sensor_num <- (substr(files, 1, 8))
  deployed <- deploy$sensor
  
  deployed_and_collected <- intersect(sensor_num, deployed)
  
  for (i in 1:length(deployed_and_collected)) {
  #i <- 26
    chosen <- paste0(deployed_and_collected[i], ".csv") #line to add i
    skip_to_next <- FALSE
    
  
      path_to_chosen <- paste0(folder, chosen)
      data <- tidy_hobo_data(path_to_chosen)
      
      #subset deployment information
      deployment_info <- dplyr::filter(deploy, sensor == deployed_and_collected[i])
      
      wet <- classify_wetdry(
        data,
        classify_var = "condUncal",
        method = "absolute",
        threshold = 10
      )
      #remove dates before and after start and end date of deployment
      wet_filt <- wet %>% 
        bind_cols(deployment_info) %>% 
        # mutate(start = deployment_info$start,
        #        end = deployment_info$end)
        dplyr::filter(datetime > start) %>%
        dplyr::filter(datetime < end) #%>%
        #bind_cols(deployment_info)
      
      
      if (i == 1)
        alldat <- wet_filt
      if (i > 1)
        alldat <- rbind(alldat, wet_filt)
}
  
  #create new column that indicates if every sensor is deployed or not
  alldat$all_deployed <- 0
  
  alldat$all_deployed[alldat$datetime > max(alldat$start) &
                        alldat$datetime <= min(alldat$end)] <- 1
  
  readout <- alldat %>% dplyr::select(-start, -end)
  
  return(readout)
}

```

```{r figuring-tz}
# figuring out timezone
figuring <- read_csv(
  file.path("./ZZloggers_9_21_23/", 
            "20011689.csv"), 
  skip = 1, 
  col_select = c(2:4))
colnames(figuring) <- c("datetime", "temp_F", "RC")


# both 
figuring %>% 
  mutate(
    # datetime from HOBOware; GMT - 4 hrs
    datetime_formatted = mdy_hms(datetime),
    datetime_formatted2 = force_tz(datetime_formatted, "US/Eastern"),
         tz_1 = tz(datetime_formatted2)) 
# Data from HOBOware is in EST already
```
```{r old read_deployment function}
# OLD FUNCTION
#inputs: folder, ws, deployment
read_deployment <- function(folder, wshed1, deployment1){
#file location
# folder = "./W3loggers_7_18_23/"
# wshed1 = "W3"
# deployment1 = 1
# i = 1
files <- list.files(folder, pattern = ".csv")
#deployment info
deploy <- read.csv("./Fieldwork_formatted_summer23/All_deployments.csv")
#filter values must have different names than colnames
w3 <- filter(deploy, wshed == wshed1 & deployment == deployment1)
w3_n <- na.omit(w3)
w3_n$start <- mdy_hm(w3_n$start)
w3_n$end <- mdy_hm(w3_n$end)

sensor_ids <- as.character(w3_n$sensor)
sensor_num <- (substr(files, 1, 8))



# r <- c(sensor_ids, sensor_num)
# r
# length(unique(r))

for(i in 1:length(sensor_num)){
  chosen <- paste0(sensor_num[i], ".csv") #line to add i
  skip_to_next <- FALSE

  tryCatch({
    
  path_to_chosen <- paste0(folder, chosen)
  data <- tidy_hobo_data(path_to_chosen)

  #subset deployment information
  deployment_info <- filter(w3_n, sensor == sensor_num[i])

  wet <- classify_wetdry(data,
                         classify_var = "condUncal",
                         method = "absolute",
                         threshold = 10)
  #remove dates before and after start and end date of deployment
  wet_filt <- filter(wet, datetime > deployment_info$start) %>%
    filter(datetime < deployment_info$end) %>% 
    bind_cols(deployment_info)


  if(i == 1) alldat <- wet_filt
  if(i > 1) alldat <- rbind(alldat, wet_filt)
  }, error=function(e){skip_to_next <<- TRUE})
  if(skip_to_next) { next }
}

#create new column that indicates if every sensor is deployed or not
alldat$all_deployed <- 0

alldat$all_deployed[alldat$datetime >= max(alldat$start) & alldat$datetime <= min(alldat$end)] <- 1

readout <- alldat %>% dplyr::select(-start, -end)

return(readout)
}
```

Read in data for all locations using for loop
12/17/23- successfully made more flexible function, produces error message if sensor data (.csv) is missing from the folder containing the data for that deployment.
2/25/24- STICr package not kept up to date, I need to make my own functions for it or get source code
3/16/26- came back to get everything ready to be posted to data repos, and it does not work. Also I think W3 is in eastern time and others are in another timezone. Need to figure this out and fix it for posterity.
```{r}
# W3
w3_1 <- read_deployment(folder = "./W3loggers_7_18_23/", 
                        study_shed = "W3",
                        deployment_number = 1) %>% tibble()
w3_2 <- read_deployment(folder = "./W3loggers_9_19_23/", 
                        study_shed = "W3",
                        deployment_number = 2) %>% tibble()
w3_3 <- read_deployment(folder = "./HBLoggers_11_14_23/", 
                        study_shed = "W3",
                        deployment_number = 3) %>% tibble()
all_w3 <- rbind(w3_1, w3_2, w3_3)
all_w3$datetime <- force_tz(all_w3$datetime, "US/Eastern")

# FB
fb_1 <- read_deployment(folder = "./FBloggers_7_19_23/", 
                        study_shed = "FB",
                        deployment_number = 1) %>% tibble()
fb_2 <- read_deployment(folder = "./FBloggers_9_20_23/", 
                        study_shed = "FB",
                        deployment_number = 2) %>% tibble()
fb_3 <- read_deployment(folder = "./HBLoggers_11_14_23/", 
                        study_shed = "FB",
                        deployment_number = 3) %>% tibble()
all_fb <- rbind(fb_1, fb_2, fb_3)
all_fb$datetime <- force_tz(all_fb$datetime, "US/Eastern")

# ZZ
zz_1 <- read_deployment(folder = "./ZZloggers_7_20_23/", 
                        study_shed = "ZZ",
                        deployment_number = 1) %>% tibble()
zz_2 <- read_deployment(folder = "./ZZloggers_9_21_23/", 
                        study_shed = "ZZ",
                        deployment_number = 2) %>% tibble()
zz_3 <- read_deployment(folder = "./HBLoggers_11_14_23/", 
                        study_shed = "ZZ",
                        deployment_number = 3) %>% tibble()
all_zz <- rbind(zz_1, zz_2, zz_3)
all_zz$datetime <- force_tz(all_zz$datetime, "US/Eastern")

# combine all 3
all_sheds <- rbind(all_w3, all_fb, all_zz) %>% 
  mutate(mins = minute(datetime))
write_csv(all_sheds, "./STIC_synthesis/all_23.csv")

all_sheds[which.max(all_sheds$tempC),]
all_sheds %>% 
  filter(sensor == 20011664,
         wshed == "ZZ") %>% 
  ggplot()+
  geom_line(aes(x = datetime, y = tempC))+
  geom_line(aes(x = datetime, y = condUncal*100/max(condUncal)), color = "blue")

# Something wrong with the max temp of this sensor, but does not seem to have impacted the observations of presence or absence
# all_sheds2 <- all_sheds[-(which(all_sheds$sensor == 20011664)),]
# all_sheds2[which.max(all_sheds2$tempC),]


#create spreadsheet containing all of the data to send to Danielle Hare
#write.csv(all_sheds2, "./DataForDanielle/HB_stic.csv")
#data <- read_csv("./DataForMary/HB_stic.csv")

## Now do for 2024 as well
#create dataframe for each watershed using read in data files function above
w3_24 <- read_deployment24(folder = "./summer2024/W3_summer24/", 
                        study_shed = "W3") %>% tibble()
w3_24$datetime <- force_tz(w3_24$datetime, "US/Eastern")

fb_24 <- read_deployment24(folder = "./summer2024/FB_summer24/", 
                        study_shed = "FB") %>% tibble()
fb_24$datetime <- force_tz(fb_24$datetime, "US/Eastern")

zz_24 <- read_deployment24(folder = "./summer2024/ZZ_summer24/", 
                        study_shed = "ZZ") %>% tibble()
zz_24$datetime <- force_tz(zz_24$datetime, "US/Eastern")


#combine all dataframes into one big one
all_24 <- bind_rows(w3_24, fb_24, zz_24)
write_csv(all_24, "./STIC_synthesis/all_24.csv")
```

```{r}
# compare to previous time reading in data
all_sheds %>% 
  group_by(wshed) %>% 
  summarise(count = n())
```

```{r}
binary <- c("#DB995A",
            "#586BA4"
  )
all_sheds %>% 
  #filter(wshed == "W3") %>% 
  filter(mins %in% c(0, 30)) %>% 
ggplot()+
  geom_tile(aes(x = datetime, y = ID, fill = wetdry))+
  #facet_grid(~deployment, scales = "free") + 
  scale_fill_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  labs(title = "Wetting and Drying patterns 2023",
       x = "")+
  theme_classic()+
  facet_wrap(~wshed)
```

Figure out how many sensors failed in each shed in each deployment.
sensors failed due to running out of battery, getting water logged and breaking, not providing any data when read, or being lost (only one in FB)
```{r}
#first figure out how many unique sensors I recorded being deployed:
#make loop to build table that shows how many sensors were deployed, and how many I have data for:
deploy <- read.csv("./Fieldwork_formatted_summer23/All_deployments.csv")
sheds <- c("W3", "FB", "ZZ")
deploys <- c(rep(1, 3), rep(2, 3), rep(3, 3))
#create empty dataframe to hold results
out <- data.frame("shed" = rep(sheds, 3), "deployment" = deploys, "ideal" = 0, "actual" = 0)

#for loop that calculates the number of sensors deployed
for(i in 1:length(out$shed)){
  out$ideal[i] <- length(unique(deploy$ID[deploy$deployment == out$deployment[i] & deploy$wshed == out$shed[i]]))
}

#calculate column with the actual number of sensors, or ones that i successfully got data from
for(i in 1:length(out$shed)){
  out$actual[i] <- length(unique(all_sheds$ID[all_sheds$deployment == out$deployment[i] & all_sheds$wshed == out$shed[i]]))
}
#calculate number missing for each deployment and shed
out$missing <- out$ideal - out$actual
#display result as a nicely formatted table
knitr::kable(out, format = "pipe")

```

How do we visualize this information?
2/25/24- switching from Rgeos land to terra land
```{r prep-for-mapping}
#Make a simple map for each watershed to serve as the template for downsampling
#terra uses SpatVector, read in using vect()
sheds <- vect('./HB/hbef_wsheds')
plot(sheds)
#method to subset shapefiles- use base r ways to subset
w3 <- sheds[2,]
plot(w3)

#read in shapefiles for each of the streams
w3_stream <- vect("./HB/hbstream/hb42_master_startend.shp")
fb_stream <- vect("./HB/hbstream/FB_subcatchment_flowlines.shp")
zz_stream <- vect("./HB/hbstream/ZZ_subcatchment_flowlines.shp")
#project the streams to wgs84
w3_stream_wgs <- project(w3_stream, "+proj=longlat +datum=WGS84")
fb_stream_wgs <- project(fb_stream, "+proj=longlat +datum=WGS84")
zz_stream_wgs <- project(zz_stream, "+proj=longlat +datum=WGS84")
#filter data for sensors in each shed
data_w3 <- unique(all_w3 %>% 
  dplyr::select(ID, deployment, lat, long)) 
data_fb <- unique(all_fb %>% 
  dplyr::select(ID, deployment, lat, long)) 
data_zz <- unique(all_zz %>% 
  dplyr::select(ID, deployment, lat, long))
#convert dataframe of sensor locations to vector data
points_w3 <- vect(data_w3, geom=c("long", "lat"), crs = "+proj=longlat +datum=WGS84")
points_fb <- vect(data_fb, geom=c("long", "lat"), crs = "+proj=longlat +datum=WGS84")
points_zz <- vect(data_zz, geom=c("long", "lat"), crs = "+proj=longlat +datum=WGS84")
#make plots of stream, then add points of sensor locations
#removed axes and labels
plot(w3_stream_wgs, col = "blue", axes = FALSE)
points(points_w3)

ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "blue")+
  geom_spatvector(data = points_w3)+
  theme_void()

zz_labels <- data_zz |> 
  select(ID, lat, long)

#creating reference image of sensor locations
#seasonal flow permanence at each point
#make binary columns for wet and dry
all_sheds2$binary <- 1
all_sheds2$binary[all_sheds2$wetdry == "dry"] <- 0


shed <- "W3"
#point_seasonal <- function(shed){
  one_shed <- filter(all_sheds2, 
               wshed == shed)
  IDs <- unique(one_shed$ID)

  w3_flowing <- one_shed %>% 
  group_by(ID)  %>% 
  reframe("percent_flowing" = sum(binary)/length(binary),
            "lat" = unique(lat),
            "long" = unique(long))
shed <- "FB"
#point_seasonal <- function(shed){
  one_shed <- filter(all_sheds2, 
               wshed == shed)
  IDs <- unique(one_shed$ID)

  fb_flowing <- one_shed %>% 
  group_by(ID)  %>% 
  reframe("percent_flowing" = sum(binary)/length(binary),
            "lat" = unique(lat),
            "long" = unique(long))
  
shed <- "ZZ"
#point_seasonal <- function(shed){
  one_shed <- filter(all_sheds2, 
               wshed == shed)
  IDs <- unique(one_shed$ID)

  zz_flowing <- one_shed %>% 
  group_by(ID)  %>% 
  reframe("percent_flowing" = sum(binary)/length(binary),
            "lat" = unique(lat),
            "long" = unique(long))

#w3
ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_spatvector(data = points_w3)+#aes(color = percent_flowing), size = 2)+
  geom_text_repel(data = w3_flowing, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("ideal n = ",31))+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("testw3.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
#fb
ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_spatvector(data = points_fb)+#aes(color = percent_flowing), size = 2)+
  geom_text_repel(data = fb_flowing, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("ideal n = ",30))+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("testfb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
#zz
ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_spatvector(data = points_zz)+#aes(color = percent_flowing), size = 2)+
  geom_text_repel(data = zz_flowing, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("ideal n = ",28))+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("testzz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")


plot(fb_stream_wgs, col = "blue", axes = FALSE)
points(points_fb)

plot(zz_stream_wgs, col = "blue", axes = FALSE)
points(points_zz)

#copy and pasted from lower down- trying to figure out the best way to plot
#plot to help me figure out where each sensor is
ggplot()+
  geom_tile(data = w3_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = w3_flowing, aes(x = long, y = lat, color = percent_flowing), size = 2)+
 geom_text_repel(data = w3_flowing, aes(x = long, y = lat,label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = "Seasonal Flow Permanence: W3")+
  coord_equal()

#one sensor per tributary, located centrally
#remove sensors
subset <- c(3, 7, 13, 16, 20, 23, 27, 28, 29, 30, 10)
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)

ggplot()+
  geom_tile(data = w3_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = w3_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
 geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat,label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme_void()+
  labs(title = "Seasonal Flow Permanence: W3")+
  coord_equal()





data_w3 <- unique(all_w3 %>% 
  dplyr::select(ID, deployment, lat, long)) 

test <- vect(data_w3, geom=c("long", "lat"), crs = "+proj=longlat +datum=WGS84")

plot(data_w3$lat, data_w3$long, add = TRUE)
#terra raster example
w3_rast <- rast("./HB/hbstream/w3_5m.tif")
w3_rast
crs(w3_rast)
plot(w3_rast, col = "blue")

test <- as.data.frame(w3_rast2, xy = TRUE) %>%
  na.omit()

all_flowing <- filter(all_w3, all_deployed == 1)

plot_test <- filter(all_w3, 
               datetime == "2023-07-03 16:10:00.00",
               wshed == "W3")

binary <- c("red",
            "blue"
  )

ggplot()+
  geom_tile(data=test, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = plot_test, aes(x = long, y = lat, color = wetdry), size = 2)+
  scale_color_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  geom_text(data = plot_test, aes(x = long, y = lat,label = ID), hjust=-0.00025, vjust=0.00025)+
  theme_void()+
  coord_equal()

wettest <- filter(all_w3, datetime == "2023-07-03 16:10:00")

```


Make plots for the wettest time and the driest time in all 3 watersheds
```{r}
#make binary columns for wet and dry
all_w3 <- data_23 %>% 
  filter(wshed == "W3")
all_w3$binary <- 1
all_w3$binary[all_w3$wetdry == "dry"] <- 0

spots_flowing <-  all_w3 %>% 
  group_by(datetime) %>% 
  summarise("percent_flowing" = sum(binary)/length(binary))

#maximum percentage of sensors flowing is 100%
max(spots_flowing$percent_flowing)
#times when stream was at it's maximum extent
spots_flowing$datetime[spots_flowing$percent_flowing == max(spots_flowing$percent_flowing)]
#minimum extent times
min(spots_flowing$percent_flowing)
no_flow <- spots_flowing$datetime[spots_flowing$percent_flowing == min(spots_flowing$percent_flowing)]


filter(q_23_f, DATETIME %in% no_flow)


spots_flowing$datetime[spots_flowing$percent_flowing == min(spots_flowing$percent_flowing)]

#plots for the wettest times for each watershed
plot_test <- filter(all_w3, 
               datetime == "2023-07-03 15:20:00 EDT")

ggplot()+
  geom_tile(data=test, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = plot_test, aes(x = long, y = lat, color = wetdry), size = 2)+
  scale_color_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  geom_text(data = plot_test, aes(x = long, y = lat,label = ID), hjust=-0.00025, vjust=0.00025)+
  theme_void()+
  labs(title = "Wettest extent in W3")+
  coord_equal()
#plot of the driest extent in W3

plot_test <- filter(all_w3, 
               datetime == "2023-09-21 16:00:00 EDT")
ggplot()+
  geom_tile(data=test, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = plot_test, aes(x = long, y = lat, color = wetdry), size = 2)+
  scale_color_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  geom_text(data = plot_test, aes(x = long, y = lat,label = ID), hjust=-0.00025, vjust=0.00025)+
  theme_void()+
  labs(title = "Driest extent in W3")+
  coord_equal()
```

instead of making plots for the wettest and driest times, make raster grid plots showing all times
```{r}

binary <- c("#ECD444", "#2274A5")

#remove measurements from first deployment that are not every 30 mins
plot_test <- filter(all_w3, deployment != 1)%>%
 slice(which(row_number() %% 3 == 0))
head(plot_test)

#flo accumulation to extract upstream area from
flowacc_wgs <-  project(flowacc, "EPSG:4326", method = "near")

data_w3_coords <- unique(all_w3 %>% 
  dplyr::select(lat, long))

lineup <- terra::extract(output_wgs, data_w3_coords)

plot_test2 <- filter(all_w3, deployment != 1) #%>% rbind(plot_test)
dates <- unique(plot_test2$datetime)

spots_flowing <- plot_test %>% 
  group_by(datetime) %>% 
  summarise("percent_flowing" = sum(binary)/length(binary))

ggplot(plot_test2)+
  geom_tile(aes(x = datetime, y = ID, fill = wetdry))+
  #facet_grid(~deployment, scales = "free") + 
  scale_fill_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  labs(title = "Streamflow permanence in W3")+
  theme_classic()

plot_test3 <- plot_test2 %>% 
  mutate(Depth = "Shallow") %>% 
  filter(ID %in% c(3, 7, 12, 17, 2, 4, 10, 6, 14, 15, 16, 13))
plot_test3$Depth[plot_test3$ID %in% c(3, 7, 12, 17)] <- "Deep"

binary <- c("#DB995A",
            "#586BA4"
  )
ggplot(all_w3)+
  geom_tile(aes(x = datetime, y = ID, fill = wetdry))+
  #facet_grid(~deployment, scales = "free") + 
  scale_fill_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  labs(title = "Streamflow permanence in W3",
       x = "")+
  theme_classic()
#ways to make better
#order points by distance to outlet or drainage area
#add discharge and precipitation
```


Chunk not formatted yet- determine number of combinations of flowing points
```{r}
#find all unique combinations of wet and dry sensors
hush <- dplyr::select(data, datetime, wetdry_binary, ID) 
hush2 <- pivot_wider(hush, names_from = ID, values_from = wetdry_binary)

xy.list <- as.list(as.data.frame(t(hush2[,-1])))
length(xy.list)
length(unique(data$datetime))
#unique combinations of flow
length(unique(xy.list))
```


12/20/23
Make some quick figures for meeting with Danielle
```{r}
#NOT CONVERTED FROM RASTER
#watershed 3 shape

w3_rast <- rast("./HB/hbstream/w3_5m.tif")
fb_rast <- rast("./HB/hbstream/fb_5m.tif")
zz_rast <- rast("./HB/hbstream/zz_5m.tif")
w3_rast <- 1
plot(w3_rast)
w3_rast2 <- projectRaster(w3_rast, crs = 4326)
fb_rast2 <- projectRaster(fb_rast, crs = 4326)
zz_rast2 <- projectRaster(zz_rast, crs = 4326)

w3_df <- as.data.frame(w3_rast2, xy = TRUE) %>%
  na.omit()
fb_df <- as.data.frame(fb_rast2, xy = TRUE) %>%
  na.omit()
zz_df <- as.data.frame(zz_rast2, xy = TRUE) %>%
  na.omit()

data_w3 <- unique(all_w3 %>% 
  dplyr::select(ID, deployment, lat, long))
data_fb <- unique(all_fb %>% 
  dplyr::select(ID, deployment, lat, long))
data_zz <- unique(all_zz %>% 
  dplyr::select(ID, deployment, lat, long))


ggplot()+
  geom_tile(data = w3_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = data_w3, aes(x = long, y = lat, color = as.factor(deployment)), size = 2)+
  geom_text(data = data_w3, aes(x = long, y = lat,label = ID), hjust=-0.00025, vjust=0.00025)+
  theme_void()+
  coord_equal()

ggplot()+
  geom_tile(data = fb_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = data_fb, aes(x = long, y = lat, color = as.factor(deployment)), size = 2)+
  geom_text(data = data_fb, aes(x = long, y = lat,label = ID), hjust=-0.00025, vjust=0.00025)+
  theme_void()+
  coord_equal()

ggplot()+
  geom_tile(data = zz_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = data_zz, aes(x = long, y = lat, color = as.factor(deployment)), size = 2)+
  geom_text(data = data_zz, aes(x = long, y = lat,label = ID), hjust=-0.00025, vjust=0.00025)+
  theme_void()+
  coord_equal()
```


Map of study watersheds for committee meeting
```{r}
sheds <- readOGR(dsn='./HB/hbef_wsheds')
w3 <- sheds %>% filter(WS == "WS3")
stream <- st_read("./HB/hbstream/hb42_master_startend.shp")
w3_rast <- raster("./HB/hbstream/w3_5m.tif")
w3_rast2 <- projectRaster(w3_rast, crs = 4326)
test <- as.data.frame(w3_rast2, xy = TRUE) %>%
  na.omit()

#study streams and whole network
w3_rast <- raster("./HB/hbstream/w3_5m.tif")
fb_rast <- raster("./HB/hbstream/fb_5m.tif")
zz_rast <- raster("./HB/hbstream/zz_5m.tif")
#use this layer, flow accumulation of 3500 m2 from 1m DEM of the whole valley
whole <- raster("./HB/hbstream/wholeValley1m_3500m2Thresh.tif")
test <- as.data.frame(whole, xy = TRUE) %>%
  na.omit()

stream <- st_read("./HB/hbstream/hb42_master_startend.shp")
fb <- st_read("./HB/hbstream/FB_subcatchment_flowlines.shp")
zz <- st_read("./HB/hbstream/ZZ_subcatchment_flowlines.shp")
#st_crs(stream)

st_crs(stream) <- 26919
st_crs(fb) <- 26919
st_crs(zz) <- 26919

data <- filter(all_w3, datetime == "2023-08-03 16:30:00")

binary <- c("red",
            "blue"
  )

```


Animation for whole record and subset
```{r}
#make animation of august 6 to august 27th
data <- filter(all_w3, datetime >= "2023-08-06 00:00:00") %>% 
               filter(datetime <= "2023-08-20 23:00:00")

data <- filter(all_w3, datetime >= "2023-07-19 00:00:00") %>% 
               filter(datetime <= "2023-07-22 23:00:00")
x <- ggplot(data)+
  geom_tile(data=test, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(aes(x = long, y = lat, color = wetdry), size = 2)+
  scale_color_manual(drop = FALSE,
                     values = binary,
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+

   theme_classic()+ 
  transition_manual(datetime) +
  labs(title = "Date and Time: {current_frame}")



animate(x, 
        fps = 6,
        nframes = 600,         # fewer frames for simplification
        device = "png")

anim_save("ws3_aug_storm.gif", x,
          fps = 6,
          nframes = 300)

saveHTML(animate(x, 
                 fps = 6,
                 nframes = 600,         # fewer frames for simplification
                 device = "png"), 
         navigator = ani.options("nmax") <= 100 && ani.options("interval") >= 0.05,
         img.name = "tryingtoanimate.png", 
         htmlfile = "tryingtoanimate.html")
```

Flow routing
```{r}
routes <- read.csv("w3_flowrouting.csv")


#3/18/24, doing analysis I talked about with Kevin
#plotting the change in discharge against change in connected surface area
head(all_sheds2)
Q2023 <- read_csv("weir3_weir_3.csv")
binary_col <- c("#2274A5", "#ECD444")


#creating function to determine connected surface area
#plot the network at this given time
show_network <- function(doi){
  #doi <- "2023-07-03 12:10:00"

points <- all_sheds2 |> 
  filter(datetime == doi) |> 
  filter(deployment == 1, wshed == "W3") |> 
  select(lat, long, wetdry)

points2 <-  vect(points, geom=c("long", "lat"), crs = "+proj=longlat +datum=WGS84")

ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = points, 
             aes(x = long, y = lat, fill = wetdry), 
             pch = 21,
             color = "black",
             size = 3)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  #scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  #labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_fill_manual(drop = FALSE,
                     values = c("#ECD444", "#2274A5"),
                    breaks = c("dry", "wet"),
                    labels = c("No flow", "flowing"),
                    name = ""
                    )+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
}
#test the show network function
show_network("2023-07-03 12:20:00")

#determine how much of the network is connected to the outlet


```

```{r}
#also run Carrie's model on the discharge from this summer
#copy and pasted from QPM_analysis_9_29_23
#read in processed discharge from this summer
#units are mm/day, which is correct for the model
q_data <- read.csv("./HB/HBEF_DailyStreamflow_1956-2023.csv")
q_data$DATE <- ymd(q_data$DATE)
w3_q <- q_data |> 
  filter(WS == 3) |> 
  filter(DATE >= "2023-01-01")

#watershed boundary, already has crs
W3 <- vect('./HB/hbef_wsheds') |> 
  filter(WS == "WS3")

#flowaccumulation to provide predefined channel
flowacc_path <- "./HB/5m hydro enforced DEM/TWI_andotherlayers/flowacc.tif"
flowacc <- terra::rast(flowacc_path)
crs(flowacc) <- crs(W3)
#crop and mask flowacc
flow_crop <- terra::crop(flowacc, W3)
flow_mask <- terra::mask(flow_crop, W3)
#threshold flow accumulation raster
flow_mask[flow_mask < 700] <- NA
flow_mask[flow_mask >= 700] <- 1
#using a critical drainage area of 700 * 5 m^2, or 3500 m^2 by     default

#twi as input to logistic regression
twi_output <- "./HB/5m hydro enforced DEM/TWI_andotherlayers/twi.tif"
twi <- terra::rast(twi_output)
crs(twi) <- crs(W3)
#crop and mask twi
twi_crop <- terra::crop(twi, W3)
w3_twi <- terra::mask(twi_crop, W3)

#function to calculate probability of flow raster
willit <- function(input_logtwi, inputQ){
  #b0, or intercept from Kevin's email from Carrie, might need to redo regression
  b0 <- -35.43
  #all ofther coefs from Jensen et al. 2018
  twi_coef <- 15.57
  flow_coef <- 0.12

  b1x1 <- flow_coef * inputQ
  b2x2 <- twi_coef * input_logtwi

  #logistic regression from Jensen et al. 2018
  p <- exp(b0 + b1x1 + b2x2)/(1 + exp(b0 + b1x1 + b2x2))
  return(p)
}

qlist <- w3_q %>% 
  dplyr::select(DATE, Streamflow)

probable_flow_extent <- function(qlist){
  out <- willit(log(w3_twi), qlist$Streamflow)
  #crs(out) <- 26919
  
  out[out < 0.9] <- 0
  out[out >= 0.9] <- 1
  
  #removes values that are not within pre-defined channel
  masked <- mask(out, flow_mask)
  
  g_df <- masked #%>%
    #as.points() %>%
    #as.data.frame() %>% 
    #mutate(Date = qlist$DATE)
  
  return(g_df)
}
output <- probable_flow_extent(qlist[200 ,])
plot(output)
#convert output to wgs84, 
output_wgs <-  project(output, "EPSG:4326", method = "near")
#plot(output_wgs)
#points(w3_stream_wgs)

#then extract values by points for each day at each sensor location
head(all_w3)
head(all_sheds2)

data_w3_coords <- unique(all_sheds2 %>% 
  dplyr::select(lat, long))

dates <- seq(ymd("2023-07-03"), ymd("2023-11-13"), 1)
qlist2 <- filter(qlist, DATE %in% dates)

output <- probable_flow_extent(qlist[200 ,])
lineup <- terra::extract(output_wgs, data_w3_coords)

#empty string to hold output of for loop
right <- c()

#get daily flow permanence to compare
all_sheds2$binary <- 1
all_sheds2$binary[all_sheds2$wetdry == "dry"] <- 0

#for loop to iterate through each day, and run Carrie's model
for(i in 1:length(dates)){
  output <- probable_flow_extent(qlist2[i ,])
  output_wgs <-  project(output, "EPSG:4326", method = "near")

}

#method copied from below to calculate percentage flowing, just need to convert to daily time step
list <- c("W3", "FB", "ZZ")
out <- seq(1, 3, 1)

for(i in 1:length(list)){
one_shed <- filter(all_sheds2, 
               wshed == list[i])
#seasonal flow permanence of whole watershed for
perc_flow <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)
out[i] <- perc_flow
}

percent_seasonal_flow <- data.frame("shed" = list,
                                       "perc_flow" = out)
```


```{r}
#find all unique combinations of wet and dry sensors
hush <- dplyr::select(data, datetime, wetdry_binary, ID) 
hush2 <- pivot_wider(hush, names_from = ID, values_from = wetdry_binary)

xy.list <- as.list(as.data.frame(t(hush2[,-1])))
length(xy.list)
length(unique(data$datetime))
#unique combinations of flow
length(unique(xy.list))



```

```{r}
#6/21/23 adding discharge and precip data
#read in data
q_raw <- read.csv("ws3_Q_6_21_2023.csv", skip = 2)
p_raw <- read.csv("ws3_P_6_21_2023.csv", skip = 3)
#remove dates outside of the extent of STIC deployment
q_raw$date.and.time <- mdy_hm(q_raw$date.and.time)
p_raw$date.time <- mdy_hm(p_raw$date.time)


q_date <- filter(q_raw, date.and.time >= "2023-06-06 14:00:00") %>% 
  filter(date.and.time <= "2023-06-14 11:00:00") %>% 
  rename("datetime" = "date.and.time")

p_date <- filter(p_raw, date.time >= "2023-06-06 14:00:00") %>% 
  filter(date.time <= "2023-06-14 11:00:00")
#convert inches to mm
p_date$mm <- p_date$inches * 25.4
p_date$day <- date(p_date$date.time)

p2 <- p_date %>% 
  group_by(day) %>% 
  summarise(daily_mmhr = sum(mm)/24)



ggplot(p2, aes(x = day, y = daily_mmhr))+
  geom_bar(stat = "identity")+
  theme_classic()+
  #transition_time(day) +
  labs(x = "", y = "Precip (mm/hr)", title = "Precipitation from June 6 to June 14")

ggplot(q_date, aes(x = datetime, y = mm.hr))+
  geom_line()+
  theme_classic()+
  labs(x = "Date", y = "Discharge (mm/hr)", title = "Discharge from June 6 to June 14")
q_date2 <- q_date
q_date2$datetime <- as.Date(q_date$datetime)


ggplot()+
  geom_bar(data = p2, aes(x = day, y = daily_mmhr), stat = "identity")+
  geom_line(data = q_date2, aes(x = datetime, y = mm.hr))+
  theme_classic()+
  #transition_time(day) +
  labs(x = "", y = "mm/hr", title = "Discharge and Precip from June 6 to June 14")
```

```{r}
#1/29/24
#Doing analysis from warix et al. 2021
#calculate seasonal flow permanence for each watershed
list <- c("W3", "FB", "ZZ")
out <- seq(1, 3, 1)

for(i in 1:length(list)){
one_shed <- filter(all_sheds2, 
               wshed == list[i])
#seasonal flow permanence of whole watershed for
perc_flow <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)
out[i] <- perc_flow
}

percent_seasonal_flow <- data.frame("shed" = list,
                                       "perc_flow" = out)
```

```{r}

  
#watershed 3 shape

w3_rast <- raster("./HB/hbstream/w3_5m.tif")
fb_rast <- terra::rast("./HB/hbstream/fb_5m.tif")
zz_rast <- terra::rast("./HB/hbstream/zz_5m.tif")

#w3_rast2 <- projectRaster(w3_rast, crs = "4326")
#fb_rast2 <- projectRaster(fb_rast, crs = 4326)
#zz_rast2 <- projectRaster(zz_rast, crs = 4326)

w3_df <- as.data.frame(w3_rast, xy = TRUE) %>%
  na.omit()
fb_df <- as.data.frame(fb_rast, xy = TRUE) %>%
  na.omit()
zz_df <- as.data.frame(zz_rast, xy = TRUE) %>%
  na.omit()

data_w3 <- unique(all_w3 %>% 
  dplyr::select(ID, deployment, lat, long))
data_fb <- unique(all_fb %>% 
  dplyr::select(ID, deployment, lat, long))
data_zz <- unique(all_zz %>% 
  dplyr::select(ID, deployment, lat, long))


ggplot()+
  geom_tile(data = w3_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = w3_flowing, aes(x = long, y = lat, color = percent_flowing), size = 2)+
 # geom_text_repel(data = w3_flowing, aes(x = long, y = lat,label = signif(percent_flowing, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = "Seasonal Flow Permanence: W3")+
  coord_equal()

ggplot()+
  geom_tile(data = fb_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = fb_flowing, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  geom_text_repel(data = fb_flowing, aes(x = long, y = lat,label = signif(percent_flowing, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = "Seasonal Flow Permanence: FB")+
  coord_equal()


ggplot()+
  geom_tile(data = zz_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = zz_flowing, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  geom_text_repel(data = zz_flowing, aes(x = long, y = lat,label = signif(percent_flowing, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = "Seasonal Flow Permanence: ZZ")+
  coord_equal()


```

```{r}
#Now randomly subset the sensors by 10%, 50%, and 90% fewer sensors randomly. See how much the percentages change

#filter to just one watershed
boot <- function(shed, iterations, percent_retained){
  out <- seq(1, iterations, 1)

  one_shed <- filter(all_sheds2, 
                     wshed == shed)
  #for loop to bootstrap n = iterations times
  for(i in 1:iterations){
  
    IDs <- unique(one_shed$ID)
    set.seed(i)
    sampled <- sample(IDs, round(percent_retained * length(IDs)))
  
    pruned <- one_shed %>% 
      filter(ID %in% sampled)

    #seasonal flow permanence of whole watershed for
    perc_flow <- length(pruned$wetdry[pruned$wetdry == "wet"]) / length(pruned$wetdry)
    out[i] <- perc_flow
  }

  avg <- mean(out)
  sd <- sd(out)

final <- paste0(signif(avg, 2)," +/- ",signif(sd, 2))
}

w3_boot <- boot("W3", 100, 0.9)
fb_boot <- boot("FB", 100, 0.9)
zz_boot <- boot("ZZ", 100, 0.9)

w3_boot <- boot("W3", 100, 0.5)
fb_boot <- boot("FB", 100, 0.5)
zz_boot <- boot("ZZ", 100, 0.5)

w3_boot <- boot("W3", 100, 0.1)
fb_boot <- boot("FB", 100, 0.1)
zz_boot <- boot("ZZ", 100, 0.1)


list <- c("W3", "FB", "ZZ")
ninety <- data.frame("shed" = list, "perc_flow" = c(w3_boot, fb_boot, zz_boot))

#final calculation should be difference between the actual and the randomly sampled/reduced
```

```{r}
boot2 <- function(shed, iterations, percent_retained){
  out <- seq(1, iterations, 1)

  one_shed <- filter(all_sheds2, 
                     wshed == shed)
  #for loop to bootstrap n = iterations times
  for(i in 1:iterations){
  
    IDs <- unique(one_shed$ID)
    set.seed(i)
    sampled <- sample(IDs, round(percent_retained * length(IDs)))
  
    pruned <- one_shed %>% 
      filter(ID %in% sampled)

    #seasonal flow permanence of whole watershed for
    perc_flow <- length(pruned$wetdry[pruned$wetdry == "wet"]) / length(pruned$wetdry)
    out[i] <- perc_flow
  }

  avg <- mean(out)
  sd <- sd(out)

final <- paste0(signif(avg, 2)," +/- ",signif(sd, 2))
}

  

shed <- "ZZ"
#point_seasonal <- function(shed){
  one_shed <- filter(all_sheds2, 
               wshed == shed)
  IDs <- unique(one_shed$ID)
  
  sampled <- sample(IDs, round(0.5 * length(IDs)))


  zz_flowing <- one_shed %>% 
  filter(ID %in% sampled) %>% 
  group_by(ID)  %>% 
  reframe("percent_flowing" = sum(binary)/length(binary),
            "lat" = unique(lat),
            "long" = unique(long))
  
ggplot()+
  geom_tile(data = zz_df, aes(x=x, y=y), fill = "lightgrey")+
  geom_point(data = zz_flowing, aes(x = long, y = lat, color = -percent_flowing), size = 2)+
  geom_text_repel(data = zz_flowing, aes(x = long, y = lat,label = signif(percent_flowing, 2)))+
  #scale_color_viridis_c()+
  theme_void()+
  labs(title = "Seasonal Flow Permanence: ZZ, 50% of sensors")+
  coord_equal()
```

Testing different logical sampling schemes

Reference plots
```{r reference-plots}
#plot to help me figure out where each sensor is
#creating reference image of sensor locations
#w3
w <- ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_spatvector(data = points_w3)+#aes(color = percent_flowing), size = 2)+
  geom_text_repel(data = w3_flowing, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("ideal n = ",31))+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
#fb
f <- ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_spatvector(data = points_fb)+#aes(color = percent_flowing), size = 2)+
  geom_text_repel(data = fb_flowing, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("ideal n = ",30))+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
#zz
z <- ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_spatvector(data = points_zz)+#aes(color = percent_flowing), size = 2)+
  geom_text_repel(data = zz_flowing, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("ideal n = ",28))+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
```

Reference flowing percentage
```{r}
subset <- seq(1, 31, 1)
one_shed <- filter(all_sheds2, 
               wshed == "W3")
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)
high_col <- "#2274A5"
low_col <- "#ECD444"

ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = w3_flowing_subset, 
             aes(x = long, y = lat, fill = percent_flowing), 
             pch = 21,
             color = "black",
             size = 3)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  #scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_fill_gradient(high = high_col, low = low_col,
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("w3_2.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#fb
subset <- seq(1, 30, 1)
one_shed <- filter(all_sheds2, 
               wshed == "FB")
fb_flowing_subset <- filter(fb_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = fb_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("fb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#zz
subset <- seq(1, 28, 1)
one_shed <- filter(all_sheds2, 
               wshed == "ZZ")
zz_flowing_subset <- filter(zz_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = zz_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("zz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
```

One sensor per tributary, centrally located
```{r}
#one sensor per tributary, located centrally
#w3
subset <- c(3, 7, 13, 16, 20, 23, 27, 28, 29, 30, 10)
one_shed <- filter(all_sheds2, 
               wshed == "W3",
               ID %in% subset)
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = w3_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("w3_2.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#fb
subset <- c(1, 2, 6, 13, 17, 20, 25, 23, 27, 31)
one_shed <- filter(all_sheds2, 
               wshed == "FB",
               ID %in% subset)
fb_flowing_subset <- filter(fb_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = fb_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("fb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#zz
subset <- c(2, 4, 8, 13, 11, 17, 16, 23, 25, 26, 27)
one_shed <- filter(all_sheds2, 
               wshed == "ZZ",
               ID %in% subset)
zz_flowing_subset <- filter(zz_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = zz_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("zz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
```

beginning and end of tribs
```{r}
# Sensors at the beginning and end of tribs
#w3
subset <- c(1, 4, 5, 11, 14, 8, 15, 17, 18, 21, 22, 26, 24, 10, 29, 30)
one_shed <- filter(all_sheds2, 
               wshed == "W3",
               ID %in% subset)
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = w3_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("w3_2.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#fb
subset <- c(1, 2, 14, 11, 21, 20, 24, 23, 29, 22, 31)
one_shed <- filter(all_sheds2, 
               wshed == "FB",
               ID %in% subset)
fb_flowing_subset <- filter(fb_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = fb_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("fb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#zz
subset <- c(1, 3, 9, 12, 11, 14, 15, 21, 20, 22, 26, 25, 27, 28)
one_shed <- filter(all_sheds2, 
               wshed == "ZZ",
               ID %in% subset)
zz_flowing_subset <- filter(zz_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = zz_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("zz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
```


easiest to access tribs
```{r}
# easiest to access tribs
subset <- c(31, 24, 22, 21, 15, 14, 1)
one_shed <- filter(all_sheds2, 
               wshed == "W3",
               ID %in% subset)
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = w3_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("w3_2.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#fb
subset <- c(seq(1, 30, 1))
exclude <-  c(1, 2, 3, 4, 11, 12, 24, 23, 25)
subset <- subset[! subset %in% exclude]
one_shed <- filter(all_sheds2, 
               wshed == "FB",
               ID %in% subset)
fb_flowing_subset <- filter(fb_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = fb_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("fb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#zz
subset <- c(seq(1, 28, 1))
exclude <-  c(21, 18, 17, 14, 16, 19, 20, 22, 23, 24, 13, 15)
subset <- subset[! subset %in% exclude]
one_shed <- filter(all_sheds2, 
               wshed == "ZZ",
               ID %in% subset)
zz_flowing_subset <- filter(zz_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = zz_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("zz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
```

every other sensor
```{r}
# every other sensor
subset <- seq(1, 31, 2)
one_shed <- filter(all_sheds2, 
               wshed == "W3",
               ID %in% subset)
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = w3_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("w3_2.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#fb
subset <- c(seq(1, 30, 2))

one_shed <- filter(all_sheds2, 
               wshed == "FB",
               ID %in% subset)
fb_flowing_subset <- filter(fb_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = fb_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("fb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#zz
subset <- c(seq(1, 28, 2))

one_shed <- filter(all_sheds2, 
               wshed == "ZZ",
               ID %in% subset)
zz_flowing_subset <- filter(zz_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = zz_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("zz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
```

other combination of every other sensor
```{r}
# every other sensor
subset <- seq(2, 31, 2)
one_shed <- filter(all_sheds2, 
               wshed == "W3",
               ID %in% subset)
w3_flowing_subset <- filter(w3_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = w3_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = w3_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("w3_2.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#fb
subset <- c(seq(2, 30, 2))

one_shed <- filter(all_sheds2, 
               wshed == "FB",
               ID %in% subset)
fb_flowing_subset <- filter(fb_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = fb_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = fb_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("fb.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")

#zz
subset <- c(seq(2, 28, 2))

one_shed <- filter(all_sheds2, 
               wshed == "ZZ",
               ID %in% subset)
zz_flowing_subset <- filter(zz_flowing, ID %in% subset)
prop <- length(one_shed$wetdry[one_shed$wetdry == "wet"]) / length(one_shed$wetdry)


ggplot()+
  geom_spatvector(data = zz_stream_wgs, color = "grey", lwd = 1.5)+
  geom_point(data = zz_flowing_subset, aes(x = long, y = lat, color = percent_flowing), size = 2)+
  #geom_text_repel(data = w3_flowing_subset, aes(x = long, y = lat, label = ID))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray")+
  theme_void()+
  labs(title = paste0("n = ",length(subset), ", tot % = ", signif(prop, 2)))+
  scale_colour_gradient(high = "darkslategray1", low = "darkslategray",
                        breaks = c(0.25, 0.5, 0.75, 1),
                        labels = c("0.25", "0.5", "0.75", "1"),
                        name = "% time flowing")+
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
ggsave("zz.png", plot = last_plot(), scale = 1, width = 3, height = 3, units = "in")
```


Geospatial analysis for Hubbard Brook study catchments
```{r}
#make sure terra and whitebox tools are loaded packages
#pour point coords, easting then northing
w3_coords <- c(281543.36, 4870433.54)
fb_coords <- c(280406.59, 4869122.57)
zz_coords <- c(277280.37, 4867436.63)

#provide path to flow direction raster for the whole valley
d8 <- "./HB/hbef_wsheds/5mhydro_6_10_23/D8pointer.tif"

#create pour point shapefile
#make dataframe
pourpoint <- data.frame("easting" = c(zz_coords[1]),
                        "northing" = c(zz_coords[2]))
#provide crs for shapefile and rasters
utm19nCRS <- CRS("+init=epsg:4326")
#convert dataframe to shapefile
r <- SpatialPointsDataFrame(pourpoint[,1:2], pourpoint[,1:2], proj4string = utm19nCRS)
#save shapefile
raster::shapefile(r, "MyShapefile.shp", overwrite = TRUE)

#perform whitebox watershed function over 5m resolution d8 pointer, using pour point
wbt_watershed(
  d8_pntr = d8,
  pour_pts = "MyShapefile.shp",
  output = "test.tif"
)

test <- raster("test.tif")
plot(test)
#find the sum of values in the raster
#first test if the raster is too big to look through in the memory
canProcessInMemory(test)
#size of W3 in ha- 42.1925 ha
#fb = 30.65
#zz = 25.25

sum(values(test), na.rm=TRUE) *25 /10000


#wbt_fill_single_cell_pits(
#                    dem = "McDonaldHollowDEM/brushDEMsm_5m_crs.tif",
 #                   output = "McDonaldHollowDEM/bmstationdem_filled.tif")

breach_output <- "./w3_dems/w3_dem_breached.tif"
wbt_breach_depressions_least_cost(
  dem = dem,
  output = breach_output,
  dist = 5,
  fill = TRUE)

fill_output <- "./w3_dems/w3_dem_filled.tif"
wbt_fill_depressions_wang_and_liu(
  dem = breach_output,
  output = fill_output
)

flowacc_output <- "./w3_dems/w3_dem_flowacc.tif"
wbt_d_inf_flow_accumulation(input = fill_output,
                            output = flowacc_output,
                            out_type = "Specific Contributing Area")

slope_output <- "./w3_dems/w3_dem_slope.tif"
wbt_slope(dem = fill_output,
          output = slope_output,
          units = "degrees")

twi_output <- "./w3_dems/w3_dem_twi.tif"
wbt_wetness_index(sca = flowacc_output, #flow accumulation
                  slope = slope_output,
                  output = twi_output)
# create pour points
WGScoor<-  MyData
> coordinates(WGScoor)=~long+lat
> proj4string(WGScoor)<- CRS("+proj=longlat +datum=WGS84")



twi <- raster(twi_output)
dem <- raster(dem)
plot(twi)


ggplot(data = filter(all_sheds, ID == 2 & wshed == "W3" & datetime <= "2023-07-10 13:00:00" & datetime >= "2023-07-03 13:00:00"))+
  geom_line(aes(x = datetime,
                y = condUncal))

```