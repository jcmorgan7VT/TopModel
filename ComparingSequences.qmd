...
#loading packages
library(pacman)
p_load(tidyverse, terra, tidyterra, whitebox, scales, wesanderson, caret, plotly,ggnewscale, sf, elevatr, patchwork, ggspatial, zoo, igraph, TSP, ggnetwork, intergraph, ggpubr, knitr, purrr, spatialEco)
...
```{r 10-cheapest-ZZ}
many_cheap <- rep("cheapest_insertion", 2)
...
# Unformatted
SOLVED! BIG PROBLEM: sequence includes all nodes, but not all nodes are present at every timestep. Sequence needs to be adjusted at each timestep to reflect the nodes that are present dynamically....
...
Chunk to find if there is any time when a different sequence is better than pk
```{r}
#summarize through time
df_summary_time <- comparison %>%
  group_by(datetime) %>%
...
df_long <- df_summary_time %>%
  ...
  tidyr::pivot_longer(cols = c(cheap_accuracy, random_accuracy, twi_accuracy), 
                      names_to = "model", values_to = "accuracy") %>% 
  mutate(diff = accuracy - pk_accuracy)
...
Figuring out weird points that are the same in each plot, when looking at accuracy versus discharge
```{r}
df_long %>% 
  inner_join(discharge_df, by = "datetime") %>% 
    filter(Q_mm_day >= 45) %>% 
...
#set of timesteps where the accuracy of every model is the same, what does this look like in space?
big_events <- df_long %>% 
  inner_join(discharge_df, by = "datetime") %>% 
    filter(Q_mm_day >= 45)

doi <- big_events$datetime

# states <- big_comb %>% 
#     filter(datetime %in% doi[3])
