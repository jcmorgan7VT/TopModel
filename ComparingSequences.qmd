...
geom_sf(data =  terra::spin(lcc, 180, 277401.8, 4867071) %>% inner_join(ID_acc_fb, by = "ID"),
        aes(fill = accuracy), size = 3, pch = 21) +
       scale_fill_gradientn(colors = c("white","lightgrey","darkgrey", "black"),
                         limits = c(0,1),
...
```{r accuracy-W3}
w3_acc <- rbind(
      produce_metrics(W3_pk_seq, "W3", "Proportion of time flowing"),
...
```{r accuracy-FB}
fb_acc <- rbind(
      produce_metrics(FB_pk_seq, "FB", "Proportion of time flowing"),
...
```{r accuracy-ZZ}
zz_acc <- rbind(
      produce_metrics(ZZ_pk_seq, "ZZ", "Proportion of time flowing"),
...
nice_table(bind_rows(w3_acc, fb_acc, zz_acc))
