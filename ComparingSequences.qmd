...
# proportion of time flowing sequences
...
## Figure 8: Maps of where sequences work in space, and time to a certain extent
...
```{r}
#original code to run a model of network activation, used to build function below based on a specific sequence
# 1. Ordered node sequence
node_sequence <- W3_pk_seq$ID[W3_pk_seq$ID %in% unlist(number_activated$deployed[1])]
...
BIG PROBLEM: sequence includes all nodes, but not all nodes are present at every timestep. Sequence needs to be adjusted at each timestep to reflect the nodes that are present dynamically....
...
df_long %>% 
  filter(datetime > start, datetime < stop) %>% 
    ggplot(aes(x = datetime, y = accuracy, color = model)) +
