...
twi_model_w3 <- calc_model_result(w3_twi_sequence$ID, "W3") %>% 
  rename("twi" = pred_out)
bfs_model_w3 <- calc_model_result(best_bfs_seqW3$ID, "W3") %>%
  rename("bfs" = pred_out)
event_model_w3 <- calc_model_result(best_event_seqW3$ID, "W3") %>%
  rename("event" = pred_out)
...
```{r discharge-colored-by-model-accuracy}
...
ggplot(aes(x = datetime, y = (Q_mm_day), color = winner)) +
  geom_point(size = 1) +
  theme_classic() +
  scale_fill_manual(values = c("#d68c45", "#247BA0", "#2c6e49",
                                "lightgrey", "maroon")) +
  scale_color_manual(values = c("#d68c45", "#247BA0",   "#2c6e49",
                                "lightgrey","maroon")) +
  labs(
