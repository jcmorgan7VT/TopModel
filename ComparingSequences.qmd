...
#loading packages
library(pacman)
p_load(tidyverse, terra, tidyterra, whitebox, scales, wesanderson, caret, plotly,ggnewscale, sf, elevatr, patchwork, ggspatial, zoo, igraph, TSP, ggnetwork, intergraph, ggpubr, knitr)
...
calc_model_result(W3_pk_seq$ID)
comparison <- input_w3 %>% 
  filter(mins %in% c(0, 30)) %>% 
  left_join(model_out, by = c("datetime", "ID")) %>% 
  select(-wshed, -mins)
...
Run on W3
```{r}
cheap_model <- calc_model_result(W3_cheap$ID) %>% rename("cheapest" = pred_out)
pk_model <- calc_model_result(W3_pk_seq$ID) %>% rename("pk" = pred_out)
random_model <- calc_model_result(W3_random$ID) %>% rename("random" = pred_out)
twi_model <- calc_model_result(w3_twi_sequence$ID) %>% rename("twi" = pred_out)
...
## Figure 7: Accuracy of all models versus accuracy
```{r W3}
...
```{r updated-function (pred_out instead of pred_binary)}
produce_metrics <- function(chain, shed, method){
# chain <- ZZ_cheap
# shed <- "ZZ"
# method = "cheapest_insertion"
...
#method just specified for ID row in output table
