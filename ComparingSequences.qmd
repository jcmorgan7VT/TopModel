...
renum_w3_event <- avg_event_w3 %>% 
  mutate(method = "Event based sequence", x = 4) %>% 
  left_join(color_key, by = "ID")
...
w3_chains <- rbind(
  ...
  scale_y_continuous(
    breaks = c(5, 24),  # Positions for the labels
    labels = c("Wets first,\ndries last", "Wets last,\ndries first") # Custom labels
  ) +
...
fb_numb_map <- ggplot()+
  ...
  geom_sf(data = fb_outline, fill = NA, color = "#397367", alpha = 0.3, lwd = 2) +
  geom_sf(data = fb_net, colour = "darkslategray3", lwd = 2) +
  geom_sf(data = fb_stic_locs_renum, aes(fill = abs(color_ID-24)+1), size = 4, pch = 21) +
  geom_sf_text(data = fb_stic_locs_renum, aes(label = color_ID), size = 2, color = "white")+
  scale_fill_viridis_c()+
  theme(rect = element_rect(fill = "transparent", color = NA))+
  theme(legend.position = "none")
...
```{r combine-chains}
(w3_chains ) / (fb_chains ) / (zz_chains ) 

(w3_numb_map / fb_numb_map / zz_numb_map) + plot_layout(heights = 1)
```
