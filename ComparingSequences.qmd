...
out_matrix <- atsp_to_stsp(edges_W3)
# 2. Write TSPLIB file
...
write_tsplib(out_matrix, "~/Documents/concorde/myinstance.tsp")
...
con_big <- read_concorde_sol("~/Documents/concorde/TSP/mysolution.tour", edges_W3)

concon2 <- tibble(node_ID = con_big, sequence = seq(1, length(con_big), 1)) %>% 
  left_join(key2, by = "node_ID") %>%
  rename(ID = sensor_ID) %>% 
  select(ID, sequence)
...
#both of these are the same, the concorde solution to my TSP
concon
concon2

opt_routes <- 
  tibble(node_ID = con_big) %>% 
  left_join(key2, by = "node_ID") %>% 
...
# Example usage:
# Suppose cost_df is your n x n dataframe of ATSP costs
# cost_mat <- as.matrix(cost_df)
# sym <- atsp_to_stsp(cost_mat, dummy_cost = 0)
# write_tsplib(sym, "instance.tsp")
# (Run Concorde externally on instance.tsp to get instance.sol)
# tour <- read_concorde_sol("instance.sol", nrow(cost_mat))
...
W3_random <- tibble(ID = sample(W3_IDs),
                    sequence = seq(1, length(W3_IDs), 1))
#running model
...
df_summary_time_w3 <- df_summary_time_w3 %>%
...
#fb_map <- 
ggplot()+
  geom_spatraster(data = t(flip(hill)))
  ...
  scale_fill_gradientn(colors = c("black", "gray9", "gray48","lightgray", "white"))+
    new_scale_fill() +
  geom_spatraster(data = t(flip(crop1)), alpha = 0.5)+
    geom_sf(data = terra::spin(fb_outline, 90, 279933.5, 4869173), fill = NA, color = "#397367", alpha = 0.3, lwd = 2) +
  geom_sf(data = terra::spin(fb_net, 90, 279933.5, 4869173), colour = "darkslategray3", lwd = 2) +
    geom_sf(data = terra::spin(lcc_fb, 90, 279933.5, 4869173), colour = "midnightblue", pch = 19, size = 2) +
...
## Figure 6: Hydrograph, colored by which sequence works the best
NEED TO FIX COLORS
```{r}
...
inner_join(big_comb, by = "datetime") #%>% 
  ggplot(aes(x = datetime, y = Q_mm_day, color = unique_sol, alpha = accuracy)) +
  #geom_point() +
  geom_point(size = 1)+
  theme_classic()+
  scale_color_manual(values = c(four_colors, "grey"))+
  labs(title = "Most accurate model along hydrograph", y = "Discharge (mm/day)", x = "")+
  facet_wrap(~ unique_sol)

```

## Figure 7: Accuracy of all models versus pk accuracy
```{r W3}
...
df_long2 %>% 
  ...
  geom_point(aes(x = pk_accuracy, 
                 y = accuracy, 
                 size = Q_mm_day, shape = event_type, color = model),
             alpha = 0.5)+
...
ID_acc_w3 <- comparison_w3 %>%
  ...
  select(ID, cheap_accuracy, pk_accuracy, random_accuracy, twi_accuracy) %>%
  tidyr::pivot_longer(cols = c(cheap_accuracy,pk_accuracy, random_accuracy, twi_accuracy), 
                      names_to = "model", values_to = "accuracy")
...
W3_rot <- ggplot()+
  ...
  geom_sf(data = w3_net, colour = "grey", lwd = 1) +
geom_sf(data = w3_stic_locs_r %>% inner_join(ID_acc_w3, by = "ID"), 
        aes(fill = accuracy), size = 2, pch = 21) +
  scale_fill_gradient(low = "white", high = "black")+
  #scale_shape_manual(values = c(21, 24, 22))+
  theme(rect = element_rect(fill = "transparent", color = NA),
...
w3_explain <- rbind(W3_cheap %>% mutate(model = "cheapest"),
      W3_pk_seq %>% mutate(model = "pk")) %>% #,
     # W3_random %>% mutate(model = "random"),
     # W3_twi_seq %>% mutate(model = "twi")) %>% 
  left_join(w3_topo_norm, by = "ID", relationship = "many-to-many") %>% 
...
## Figure 9: Change in Q, versus accuracy
```{r dQ}
...
```{r}
#print topo metrics calculated in earlier section
...
zz_acc <- rbind(produce_metrics(ZZ_cheap, "ZZ", "cheapest_insertion"),
      ...
      produce_metrics(ZZ_random, "ZZ", "random"),
...
# Should I run the analysis again, but separating the two summers? Or by season? Or by wetting and drying?
```{r}
