...
rbind(#mutate(W3_cheap, method = "cheapest_insertion",x = 1),
      ...
      mutate(w3_twi_sequence, method = "twi", x = 2),
      mutate(convert_to_IDseq(new_seq_w3), method = "new", x = 3)
      #mutate(convert_to_IDseq(opt_routes_w3), method = "con", x = 6)
      ) %>% 
  ggplot()+
  geom_point(aes(x = x, y = sequence, color = ID), size = 10)+
  geom_text(aes(x = x, y = sequence, label = ID), color = "white")+
  labs()
```
