...
#loading packages
library(pacman)
p_load(tidyverse, terra, tidyterra, whitebox, scales, wesanderson, caret, plotly,ggnewscale, sf, rgeoboundaries, elevatr, patchwork, ggspatial, zoo, igraph, TSP)
...
W3_all_combos <- calc_props(all_combos_routes, "W3")
```
...
combos <- unique(c(routes_zz$up, routes_zz$down))
zzz <- length(combos)
rep(combos, zzz)
all_list <- c()
for(z in 1:zzz){
