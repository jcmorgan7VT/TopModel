...
p_load(tidyverse, terra, tidyterra, whitebox, scales, wesanderson, caret, plotly,ggnewscale, sf, elevatr, patchwork, ggspatial, zoo, igraph, TSP, ggnetwork, knitr, #intergraph, ggpubr, #purrr, spatialEco, 
       hydroEvents,ggbreak,
       #maptools, # no current version
       rempsyc, # make nice table
       ggpubr, # some functions for graphing, central tendency
       ggbreak # function for axis break
)
...
paste0(tolower(substr("ZZ", 1, 2)), "_")

data_23 %>% 
  select(wshed, ID, lat, long, deployment) %>% 
  filter(deployment == 1) %>% 
  unique() %>% 
  mutate(ID = case_when(
      wshed == "W3" ~ paste0(tolower(substr(wshed, 1, 1)), "3_", ID),
      wshed == "ZZ" ~ paste0(tolower(substr(wshed, 1, 2)), "_", ID),
      wshed == "ZZ" ~ paste0(tolower(substr(wshed, 1, 2)), "_", ID))) %>% 
  select(ID, lat, long) %>% View()
  filter(ID %in% c("w3_4", "w3_25", "w3_26", "w3_30", "w3_31", "zz_5", "zz_6"))
  View()
...
convert_to_IDseq2 <- function(input){ 
  #expecting input that is a df of up/down structure
  # different than other function, where first row is the most upstream pair, or last in sequence
  input2 <- input#input[rev(seq_len(nrow(input))), ]
  #make list in order of parent-child sequence
  IDs <- c(input2$up[1], input2$down)
  #make sequence ID column
  sequence <- seq(1, length(input2$up) + 1, 1)
 
  convert <- tibble(ID = as.numeric(substr(IDs,3,4)),
           sequence = sequence)
...
SOMEHOW REMOVED THIS AT SOME POINT????
```{r define-functions}
...
w3_discharge_bind <- rbind(q_23_bind, q_24_bind)
...
Might not need this chunk? It has an error I do not understand
...
q_23_plotting %>% 
  ...
  arrange(datetime) %>% 
  unique() %>% 
  dplyr::mutate(lab = as.numeric(row_number())) %>% 
...
```{r process-dilutions}
source("calcQ.R")
calcQ("./salt_dilutions/FB/FB_8_1") %>% dplyr::mutate("shed" = "FB")
q_combined <- rbind(
...
#FB_air <- 
  read_csv("./PressureTransducers_summer24/FB_air.csv", skip = 1) #%>% 
  select(2:3) %>% 
  rename(DATETIME = 1,
         pressure_psi_air = 2) %>% 
  mutate(DATETIME = mdy_hms(DATETIME)) %>% 
...
w3_uaa <- 
  terra::extract(rast(flowacc_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("uaa" = `X10mdem_flowacc`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

w3_tpi <- terra::extract(rast(tpi_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("tpi" = `X10mdem_tpi`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

w3_twi <- terra::extract(rast(twi_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("twi" = `X10mdem_twi`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

w3_slope <- terra::extract(rast(slope_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
...
fb_uaa <- terra::extract(rast(flowacc_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("uaa" = `X10mdem_flowacc`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

fb_tpi <- terra::extract(rast(tpi_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("tpi" = `X10mdem_tpi`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

fb_twi <- terra::extract(rast(twi_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("twi" = `X10mdem_twi`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

fb_slope <- terra::extract(rast(slope_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("slope" = `X10mdem_slope`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)
...
m10 <- aggregate(m1, 10)
#save raster, because whitebox wants it is a files location instead of an object in R
writeRaster(m10, "./zz_dems/10mdem.tif", overwrite = TRUE)


breach_output <- "./zz_dems/10mdem_breach.tif"
wbt_breach_depressions_least_cost(
  dem = "./zz_dems/10mdem.tif",
  output = breach_output,
  dist = 10,
  fill = TRUE)

fill_output <- "./zz_dems/10mdem_fill.tif"
wbt_fill_depressions_wang_and_liu(
  dem = breach_output,
  output = fill_output
)
...
#flow accumulation/drainage area
flowacc_output <- "./zz_dems/10mdem_flowacc.tif"
wbt_d_inf_flow_accumulation(input = fill_output,
                            output = flowacc_output,
                            out_type = "Specific Contributing Area")
#Slope
slope_output <- "./zz_dems/10mdem_slope.tif"
wbt_slope(dem = fill_output,
          output = slope_output,
          units = "degrees")
plot(rast(slope_output))
#calculate TPI
tpi_output <- "./w3_dems/10mdem_tpi.tif"
wbt_relative_topographic_position(
    dem = fill_output, 
...
zz_uaa <- terra::extract(rast(flowacc_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("uaa" = `X10mdem_flowacc`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

zz_tpi <- terra::extract(rast(tpi_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("tpi" = `X10mdem_tpi`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

zz_twi <- terra::extract(rast(twi_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("twi" = `X10mdem_twi`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)

zz_slope <- terra::extract(rast(slope_output), locs_shape, ID = FALSE, bind = TRUE) %>% 
  rename("slope" = `X10mdem_slope`) %>% 
  as_tibble() %>% 
  pivot_longer(-ID)
...
All code for analysis below, but skippable; results preserved in saved .csv files.
  
## Identify events and define sequences
Originally in DetermineActualSequence, brought here in the great reconciliation of scripts.
...
# as opposed to identifying events, this is taking those events and using them to create a sequence
ttt <- 
input_w3 %>%
...
# Define sequences to then test using my suite of functions
all_events23 <- all_events

# df with all event sequences for 2023 in W3
all_events23_sequences <- all_events %>% 
  select(-start_of_flow)

# save output, so I can skip analysis later
write_csv(all_events23_sequences, "W3event_sequences23.csv")

# add the end of each event, used for plotting later maybe?
...
input_w3  %>% filter(datetime < stop_23,
  ...
  left_join(all_events23_sequences) %>% 
...
# all_events is the output of this loop
all_events24 <- all_events

# all sequences from events for W3, 2024
all_events24_sequences <- all_events %>% 
  select(-start_of_flow)

# save output so I can skip it later
...
### FB
```{r sequence}
FBall_events <- read_csv("FB_events.csv") %>% 
...
input_fb %>%
  # filter(datetime < stop_23,
  #                    datetime >= start_23) %>% 
  inner_join(FBall_events,  by = c("datetime")) %>% 
    filter(binary == 1) %>% 
    group_by(ID, event_id) %>% 
    summarise(start_of_flow = min(datetime)) %>% 
    arrange(start_of_flow) %>% 
...
# Define sequences to then test using my suite of functions
all_eventsFB <- all_events

all_eventsFB_sequences <- all_eventsFB %>% 
  select(-start_of_flow)
...
### ZZ
```{r sequence}
ZZall_events <- read_csv("ZZ_events.csv") %>% 
  dplyr::rename(datetime = DATETIME)

# determine the sequence that sensors activate during events
ttt <- 
input_zz %>%
  # filter(datetime < stop_23,
  #                    datetime >= start_23) %>% 
  inner_join(ZZall_events,  by = c("datetime")) %>% 
    filter(binary == 1) %>% 
    group_by(ID, event_id) %>% 
    summarise(start_of_flow = min(datetime)) %>% 
    arrange(start_of_flow) %>% 
...
# Define sequences to then test using my suite of functions
all_eventsZZ <- all_events

all_eventsZZ_sequences <- all_eventsZZ %>% 
  select(-start_of_flow)
...
Old method to determine the best sequence of the event sequences
Instead of looking at all of the event sequences and finding the best one, I am using pairwise comparison to determine the "average" sequence.
```{r accuracy-of-individual-event-seqs-W3}
# make loop and function to iterate through many sequences, and determine their accuracy through time
# only for W3
# apply_many <- function(input){
#   model_out <- calc_model_result(input$ID, "W3")
#   
...
#   drop_na()
#   
#   comparison_w3$pred_eval <- mapply(get_eval_label, 
#                                     comparison_w3$binary, 
#                                     comparison_w3$pred_out)
# 
#   df_summary_time_w3 <- comparison_w3 %>%
#   group_by(datetime) %>%
#   summarize(
#     pred_correct = sum(pred_eval == "correct"),
#     total = n(),
#     pred_accuracy = pred_correct / total
#   )
#   
#   df_long_w3 <- df_summary_time_w3 %>%
#   select(datetime, contains("accuracy")) %>% 
#   tidyr::pivot_longer(cols = c(pred_accuracy),
...
for(i in 1:length(unique(event_seqsW3_23$event_id))){
out <- event_seqsW3_23 %>% 
  filter(event_id == i) %>% 
  select(-event_id) %>%
produce_metrics(., "W3", i)

  if(i <= 1) allout <- out
  if(i > 1) allout <- rbind(allout, out)
}
event_seqsW3_23_accuracy <- allout
event_seqsW3_23_accuracy %>% 
  filter(Accuracy == max(Accuracy))

for(i in 1:length(unique(event_seqsW3_24$event_id))){
out <- event_seqsW3_24 %>% 
  filter(event_id == i) %>% 
  select(-event_id) %>%
produce_metrics(., "W3", i)
...
event_seqsW3_24_accuracy <- allout
event_seqsW3_24_accuracy %>% 
  filter(Accuracy == max(Accuracy))

# Which individual sequence has the highest accuracy for the whole record?
event_seqsW3_23_accuracy %>% 
  filter(Accuracy == max(Accuracy))

## Old code of defining the best sequence
# best_event_seqW3 <- 
#   event_seqsW3_23 %>% 
#   filter(event_id == 2) %>% 
#   select(ID, Sequence)
```
```{r accuracy-of-individual-event-seqs-FB}
...
for(i in 1:length(unique(all_eventsfb_sequences$event_id))){
out <- all_eventsfb_sequences %>% 
  filter(event_id == i) %>% 
  select(-event_id) %>%
produce_metrics(., "FB", i)

  if(i <= 1) allout <- out
  if(i > 1) allout <- rbind(allout, out)
}
event_seqsFB_accuracy <- allout
event_seqsFB_accuracy
event_seqsFB_accuracy %>% 
  filter(Accuracy == max(Accuracy))

#only 14 sensors in sequence
best_event_seqFB <- 
  all_eventsfb %>% 
  filter(event_id == 14) %>% 
  select(ID, Sequence)
...
## Once sequences are defined, format and calc average
```{r skip-analysis-read-results}
#### W3 ####
# created in DetermineActualSequence.qmd,a csv containing the activation sequence for each event for W3
event_seqsW3_23 <- read_csv("W3event_sequences23.csv")
event_seqsW3_24 <- read_csv("W3event_sequences24.csv")

# combine the two years of sequences, re-number event_id
event_seqsW3 <- event_seqsW3_24 %>% 
  mutate(event_id = event_id + max(event_seqsW3_23$event_id)) %>% 
  rbind(event_seqsW3_23)

#### FB ####
# created earlier in this script
event_seqsFB <- read_csv("FBevent_sequences.csv")

#### ZZ ####
event_seqsZZ <- read_csv("ZZevent_sequences.csv")

```

## Determine average sequence, using pairwise comparison
```{r pairwise-comparison-function}

# perhaps terrible practice, but I am loading and unloading the packages within the function...
average_activation_sequence <- function(data, 
                                        sensor_col = "sensor_id", 
                                        event_col = "event_id") {
  
p_load(stringdist, DECIPHER)
  # ---- 1. Extract sequences from dataframe order ----
  data <- data[order(data[[event_col]]), ]
  seqs <- split(data[[sensor_col]], data[[event_col]])
  
  # ---- 2. Identify all unique sensors ----
  all_sensors <- sort(unique(unlist(seqs)))
  n <- length(all_sensors)
  
  # Precedence matrix: P[i,j] = fraction where i occurs before j
  P <- matrix(NA, nrow = n, ncol = n,
              dimnames = list(all_sensors, all_sensors))
  
  # ---- 3. Compute precedence probabilities ----
  for (s1 in all_sensors) {
    for (s2 in all_sensors) {
      ...
      for (seq in seqs) {
        # Only count events where both sensors appear
        if (s1 %in% seq && s2 %in% seq) {
          total <- total + 1
          if (match(s1, seq) < match(s2, seq)) wins <- wins + 1
        }
      }
      
      if (total > 0) {
        P[as.character(s1), as.character(s2)] <- wins / total
      }
    }
  }
  
  # ---- 4. Compute global precedence score for each sensor ----
  # Score = average probability sensor occurs before others
  score <- rowMeans(P, na.rm = TRUE)
  
  # ---- 5. Consensus activation sequence ----
  consensus_sequence <- names(sort(score, decreasing = TRUE))
  
  p_unload(stringdist, DECIPHER)
  # ---- 6. Return everything ----
  list(
    sequences = seqs,
    precedence_matrix = P,
...
Apply function to the three watersheds...
```{r W3}
# format data for function; expecting df (not tibble) with event_id and sensor_id columns, in the order of the sequence. All events in one large table.
data <- event_seqsW3 %>% 
...
# run averaging function on data
result <- average_activation_sequence(data)

# extract just the "average" sequence
average_event_seq <- as.numeric(result$consensus_sequence)

# format the average sequence
avg_event_w3 <- tibble(ID = average_event_seq,
       sequence = seq(1, length(average_event_seq), 1))

# report accuracy of average event sequence when modeled
tibble(ID = average_event_seq,
       Sequence = seq(1, length(average_event_seq), 1)) %>% 
produce_metrics(., "W3", "average")
...
# format the average sequence
avg_event_fb <- tibble(ID = average_event_seq,
       sequence = seq(1, length(average_event_seq), 1))

# report accuracy of average event sequence when modeled
tibble(ID = average_event_seq,
       Sequence = seq(1, length(average_event_seq), 1)) %>% 
produce_metrics(., "FB", "average")
...
# format the average sequence
avg_event_zz <- tibble(ID = average_event_seq,
       sequence = seq(1, length(average_event_seq), 1))

# report accuracy of average event sequence when modeled
tibble(ID = average_event_seq,
       Sequence = seq(1, length(average_event_seq), 1)) %>% 
produce_metrics(., "ZZ", "average")

```


```{r format-for-density-plot}
all_event <- rbind(
convert_to_updown(avg_event_w3) %>% 
  calc_props(., "W3"),
convert_to_updown(avg_event_fb) %>% 
  calc_props(., "FB"),
convert_to_updown(avg_event_zz) %>% 
  calc_props(., "ZZ"))
```
...
all_events_readyfb <- all_eventsfb %>% 
  select(-start_of_flow)

# tile plot of presence and absence data
input_fb  %>% 
  inner_join(FBall_events, by = "datetime") %>% 
  filter(event_type != "baseflow") %>% 
  left_join(all_events_readyfb) %>% 
  ggplot(aes(x = datetime))+
  geom_tile(aes(y = Sequence, fill = as.character(binary)))+
  scale_y_continuous("Observed Activation Order")+
  #facet_grid(~name, scales = "free") + 
...
create look-up table of all combinations of prop values
Do not need to implement for other two sheds I think
```{r W3}
...
lookup_table <- W3_all_combos %>% 
  ungroup() %>% 
  select(up, down, prop) %>% 
  mutate(child = as.numeric(substr(up, 3, 4)),
         parent = as.numeric(substr(down, 3, 4))) %>% 
  select(-c(up, down)) %>% 
  pivot_wider(names_from = parent, values_from = prop)
...
smart_iterate_w3_new <- read_csv("breadthfs_results.csv")
...
#  #x <- 1
#  #calc_props(routes[x], "W3")
#         out <- calc_props(routes, "FB") %>% filter(timescale == "30mins")
#         
#         kep <- out[out$prop == max(out$prop),] #find the max prop value
#         kep <- kep[1,] #if tied, keep the first one/row
#   options <- options[options != substr(kep$down[1], 3, 4)] #create new options list
...
# 
# 
# smart_iterate_fb_new <- biggest_keep
# write_csv(smart_iterate_fb_new, "breadthfs_resultsFB.csv")
```
```{r ZZ}
# Sys.time()
# 
...
#  #x <- 1
#  #calc_props(routes[x], "W3")
#         out <- calc_props(routes, "ZZ") %>% filter(timescale == "30mins")
#         
#         kep <- out[out$prop == max(out$prop),] #find the max prop value
#         kep <- kep[1,] #if tied, keep the first one/row
#   options <- options[options != substr(kep$down[1], 3, 4)] #create new options list
...
# 
# 
# smart_iterate_zz_new <- biggest_keep
# write_csv(smart_iterate_zz_new, "breadthfs_resultsZZ.csv")
```

## Skip run, read results
Already run BFS, now can just read in results in future
```{r read-csv-BFS-results}
bfsW3 <- read_csv("breadthfs_results.csv")
bfsFB <- read_csv("breadthfs_resultsFB.csv")
bfsZZ <- read_csv("breadthfs_resultsZZ.csv")


# optional plot of the density plot of prop values, compared to pk
bfsZZ %>% group_by(start) %>% 
  ggplot()+
  geom_density(data = bfsZZ, 
               aes(x = prop, y = after_stat(density), color = as.factor(start)))+
  geom_density(data = filter(all_pk, timescale == "30mins" | shed == "ZZ"))+
...
## determine average sequence
```{r W3}
# for loop to go through all of the seeds, convert to IDseq format
for(i in 1:length(unique(bfsW3$start))){
out <- bfsW3 %>% 
  filter(start == i) %>% 
  select(up, down) %>% 
  # make sure to use convert 2, so that the upstream most is not lost
  convert_to_IDseq2() %>% 
  mutate(sensor_id = ID,
         event_id = i) %>% 
  select(event_id, sensor_id)
...
# use average activation sequence function to find the average sequence from all BFS seeds
bfs_avg_seq <- average_activation_sequence(data.frame(allout))

average_bfs_seq <- as.numeric(bfs_avg_seq$consensus_sequence)

best_bfs_w3 <- tibble(ID = average_bfs_seq,
       sequence = seq(1, length(average_bfs_seq), 1))

# calculate accuracy of the average sequence
tibble(ID = average_bfs_seq,
       Sequence = seq(1, length(average_bfs_seq), 1)) %>% 
produce_metrics(., "W3", "average")

# # Original method of finding the best of the 26 BFS sequences
# for(i in 1:length(unique(bfsW3$start))){
# out <- bfsW3 %>% 
#   select(up, down) %>% 
#   convert_to_IDseq
...
#(just1[rev(seq_len(nrow(just1))), ])


```
```{r FB}
# for loop to go through all of the seeds, convert to IDseq format
for(i in 1:length(unique(bfsFB$start))){
out <- bfsFB %>% 
  filter(start == i) %>% 
  select(up, down) %>% 
  # make sure to use convert 2, so that the upstream most is not lost
...
best_bfs_fb <- tibble(ID = average_bfs_seq,
       sequence = seq(1, length(average_bfs_seq), 1))
...
```{r ZZ}
# for loop to go through all of the seeds, convert to IDseq format
for(i in 1:length(unique(bfsZZ$start))){
out <- bfsZZ %>% 
  filter(start == i) %>% 
  select(up, down) %>% 
  # make sure to use convert 2, so that the upstream most is not lost
...
best_bfs_zz <- tibble(ID = average_bfs_seq,
       sequence = seq(1, length(average_bfs_seq), 1))
...
convert_to_updown(best_bfs_zz) %>% 
  calc_props(., "ZZ")
```

```{r format-for-density-plot}
all_bfs <- rbind(
convert_to_updown(best_bfs_w3) %>% 
  calc_props(., "W3"),
convert_to_updown(best_bfs_fb) %>% 
  calc_props(., "FB"),
convert_to_updown(best_bfs_zz) %>% 
  calc_props(., "ZZ"))
```
...
```
Run on W3
Changes- make some way to run on an ensemble of sequences, and then keep the one that yields the highest accuracy?

Can report the range of accuracies for the multitude of sequences from actual and optimized sequences
```{r W3}
# running model on each sequence
pk_model_w3 <- calc_model_result(W3_pk_seq$ID, "W3") %>% 
  dplyr::rename("pk" = pred_out)
twi_model_w3 <- calc_model_result(w3_twi_sequence$ID, "W3") %>% 
  dplyr::rename("twi" = pred_out)
bfs_model_w3 <- calc_model_result(best_bfs_w3$ID, "W3") %>%
  dplyr::rename("bfs" = pred_out)
event_model_w3 <- calc_model_result(avg_event_w3$ID, "W3") %>%
  dplyr::rename("event" = pred_out)
...
comparison_w3 <- input_w3 %>% 
  ...
  left_join(bfs_model_w3, by = c("datetime", "ID")) %>% 
  left_join(event_model_w3, by = c("datetime", "ID")) %>% 
  select(-wshed, -mins) %>% 
  drop_na()
...
comparison_w3$bfs_eval <- mapply(get_eval_label, comparison_w3$binary, comparison_w3$bfs)
comparison_w3$event_eval <- mapply(get_eval_label, comparison_w3$binary, comparison_w3$event)
...
df_summary_time_w3 <- comparison_w3 %>%
  ...
  summarize(
    ...
    bfs_correct = sum(bfs_eval == "correct"),
    event_correct = sum(event_eval == "correct"),
    #con_correct = sum(con_eval == "correct"),
    total = n(),
    pk_accuracy = pk_correct / total,
    twi_accuracy = twi_correct / total,
    bfs_accuracy = bfs_correct / total,
    event_accuracy = event_correct / total
  )

df_long_w3 <- df_summary_time_w3 %>%
  select(datetime, contains("accuracy")) %>% 
  tidyr::pivot_longer(cols = c(pk_accuracy, 
                               twi_accuracy, bfs_accuracy,
                               event_accuracy),#, con_accuracy), 
                      names_to = "model", values_to = "accuracy")
...
```{r FB}
# running model on each sequence
pk_model_fb <- calc_model_result(FB_pk_seq$ID, "FB") %>% 
  dplyr::rename("pk" = pred_out)
twi_model_fb <- calc_model_result(fb_twi_sequence$ID, "FB") %>% 
  dplyr::rename("twi" = pred_out)
bfs_model_fb <- calc_model_result(best_bfs_fb$ID, "FB") %>%
  dplyr::rename("bfs" = pred_out)
event_model_fb <- calc_model_result(avg_event_fb$ID, "FB") %>%
  dplyr::rename("event" = pred_out)
...
comparison_fb <- input_fb %>% 
  ...
  left_join(bfs_model_fb, by = c("datetime", "ID")) %>% 
  left_join(event_model_fb, by = c("datetime", "ID")) %>% 
  select(-wshed, -mins) %>% 
  drop_na()
...
comparison_fb$bfs_eval <- mapply(get_eval_label, comparison_fb$binary, comparison_fb$bfs)
comparison_fb$event_eval <- mapply(get_eval_label, comparison_fb$binary, comparison_fb$event)
...
```{r ZZ}
# running model on each sequence
pk_model_zz <- calc_model_result(ZZ_pk_seq$ID, "ZZ") %>% 
  dplyr::rename("pk" = pred_out)
twi_model_zz <- calc_model_result(zz_twi_sequence$ID, "ZZ") %>% 
  dplyr::rename("twi" = pred_out)
bfs_model_zz <- calc_model_result(best_bfs_zz$ID, "ZZ") %>%
  dplyr::rename("bfs" = pred_out)
event_model_zz <- calc_model_result(avg_event_zz$ID, "ZZ") %>%
  dplyr::rename("event" = pred_out)
...
comparison_zz <- input_zz %>% 
  ...
  left_join(bfs_model_zz, by = c("datetime", "ID")) %>% 
  left_join(event_model_zz, by = c("datetime", "ID")) %>% 
  select(-wshed, -mins) %>% 
  drop_na()
...
comparison_zz$bfs_eval <- mapply(get_eval_label, comparison_zz$binary, comparison_zz$bfs)
comparison_zz$event_eval <- mapply(get_eval_label, comparison_zz$binary, comparison_zz$event)
...
#save coordinates of FB STIC locations
#writeVector(lcc_fb, "fb_stic_locs.shp")
...
#writeVector(lcc, "zz_stic_locs.shp")
#plot(lcc)
#define the rectangular area that will be shown on final map
...
#zz_map <- 
  ...
  scale_fill_continuous(high = "#27408B", low = "white",
                        ...
                        breaks = c(0, 0.5, 1),
                        limits = c(0, 1))+
        geom_sf(data = vect(zz_pour_snap))+
...
ggsave("valley.pdf", plot = valley_plot, limitsize = FALSE, units = "in", bg = NULL)
...
### combine big map with individual ones here
```{r}
design <- "
1111
2345
...
combined <- valley_plot + w3_map_f1 + fb_map + zz_map + guide_area()+ plot_layout(design = design, guides = "collect")
combined

big_comb <- combined + plot_layout(design = design, guides = "collect")
big_comb
ggsave("big_comb.pdf", big_comb)
```
...
write_csv(tibble(W3_IDs), "W3_IDs.csv")
w3_net <- vect("./carrieZigZag/w3_network.shp")
#writeVector(w3_net, "./seismic_map_exports/network.shp")
...
w3_map_f1 <- 
  ggplot()+
  theme_void()+
  geom_sf(data = w3_outline, fill = NA, color = "grey", alpha = 0.3, lwd = 2)+
  geom_sf(data = w3_stream, colour = "#9AC0CD", lwd = 1) +
  geom_sf(data = filter(w3_stic_locs_r,
                        ID %in% W3_IDs), aes(fill = pk), size = 3, pch = 21) +
  binned_scale(name = "Flow persistence",
               aesthetics = "fill",
               scale_name = "stepsn", 
               palette = function(x) c("#F0F2FA", "#B3C0E5","#4969C1", "#2A3F7A"),
...
subset <- w3_stic_locs_r %>% 
  filter(ID %in% c(11, 12, 13, 14))
...
#w3_map_f1 <- 
  ...
  geom_sf(data = w3_outline, fill = NA, color = "grey", alpha = 0.3, lwd = 2)+
  geom_sf(data = w3_stream, colour = "#9AC0CD", lwd = 1) +
  geom_sf(data = subset, aes(fill = pk), size = 3, pch = 21) +
  binned_scale(name = "Flow persistence",
               aesthetics = "fill",
               scale_name = "stepsn", 
               palette = function(x) c("#F0F2FA", "#B3C0E5","#4969C1", "#2A3F7A"),
...
#Again, but with viridis color scale
  #w3_map_f1 <- 
  ggplot()+
  theme_void()+
  geom_sf(data = w3_outline, fill = NA, color = "grey", alpha = 0.3, lwd = 2)+
  geom_sf(data = w3_stream, color = "#9AC0CD", lwd = 1) +
  geom_sf(data = subset, aes(fill = pk), size = 3, pch = 21) +
...
w3_map_f1 <- 
  ...
  #geom_spatraster(data = crop1, alpha = 0.5)+
  geom_sf(data = w3_outline, fill = NA, color = "#FFD166", alpha = 0.3, lwd = 2)+
  geom_sf(data = w3_regime, colour = "#9AC0CD", lwd = 1) +
  geom_sf(data = filter(w3_stic_locs_r,
                        ID %in% W3_IDs), aes(fill = pk), size = 3, pch = 21) +
  #geom_sf(data = dd, aes(color = (depth)), pch = 19, size = 3) +
...
fb_map <- 
  ...
  geom_sf(data = fb_net, colour = "darkslategray3", lwd = 1) +
    #geom_sf(data = lcc_fb, colour = "midnightblue", pch = 19, size = 2) +
    geom_sf(data = filter(lcc_fb,
                          ID %in% FB_IDs), aes(fill = pk), size = 3, pch = 21) +
  #geom_sf(data = dd, aes(color = (depth)), pch = 19, size = 3) +
  binned_scale(aesthetics = "fill",
               scale_name = "stepsn", 
               palette = function(x) c("#F0F2FA", "#B3C0E5","#4969C1", "#2A3F7A"),
...
zz_map <- 
  ...
  geom_sf(data = zz_net, colour = "darkslategray3", lwd = 1) +
    #geom_sf(data = lcc, colour = "midnightblue", pch = 19, size = 2) +
    geom_sf(data = filter(lcc,
                          ID %in% ZZ_IDs), aes(fill = pk), size = 3, pch = 21) +
  #geom_sf(data = dd, aes(color = (depth)), pch = 19, size = 3) +
  #geom_sf(data = zz_pour, colour = "black", pch = 8, size = 3) +
   #scale_fill_hypso_c(palette = "dem_screen", limits = c(200, 1000))+
  #geom_sf(data = dd, aes(color = (depth)), pch = 19, size = 3) +
...
zz_map
ggsave("zz_map_f1f.png", zz_map )

```
## Figure s.2: Vignettes of network wetting and drying
...
Code from determining event-based sequence
```{r}
...
input_w3  %>% filter(datetime < stop_23,
  ...
  # lims(x = c(ymd_hms("2023-07-20 16:30:00 UTC"), 
  #            ymd_hms("2023-07-24 10:30:00 UTC") - days(2)))
  #geom_vline(aes(xintercept = start_of_event))+
   # geom_vline(aes(xintercept = end_of_event))+
...
(water)/tiles + plot_layout(heights = c(1, 5))
ggsave("activation_order.png", scale = 2)
```
...
threshold_hours <- 4   # minimum sustained flow duration (hours)
max_gap_hours   <- 1   # allowable gap between observations before a run is broken

pks_ordered <- input_w3 %>%
  filter(ID %in% remainder,
         datetime > start,
         datetime < stop) %>%
  # Ensure time-sequential order within each ID
  arrange(ID, datetime) %>%
  group_by(ID) %>%
  mutate(
    # Hours since previous reading
    gap = as.numeric(difftime(datetime, 
                              lag(datetime, default = first(datetime)), 
                              units = "hours")),
    # Start a new run if: first row, currently dry, or gap exceeds tolerance
    new_run = ifelse(row_number() == 1 | 
                     binary == 0 | 
                     gap > max_gap_hours, 1, 0),
    run_id = cumsum(new_run)
  ) %>%
  # Keep only flowing observations for duration calculation
  filter(binary == 1) %>%
  group_by(ID, run_id) %>%
  summarise(
    run_start    = min(datetime),
    run_end      = max(datetime),
    # Duration from first to last observation in the run.
    # For 30-minute data, add 0.5 hr if you want to include the final interval.
    run_duration = as.numeric(difftime(run_end, run_start, units = "hours")),
    .groups      = "drop"
  ) %>%
  # Retain only runs that meet the continuous-flow threshold
  filter(run_duration >= threshold_hours) %>%
  # For each ID, keep the start of its earliest qualifying run
  group_by(ID) %>%
  summarise(start_of_flow = min(run_start), .groups = "drop") %>%
  # Order by activation and assign rank
  arrange(start_of_flow) %>%
  rowid_to_column("pk_order") %>%
  select(ID, pk_order)
...
start <- ymd_hms("2023-09-10 00:00:00")
# stop <- ymd_hms("2023-10-22 00:00:00")
stop <- ymd_hms("2023-09-16 00:00:00")
...
test <- 
  ...
  #filter(ID %in% W3_IDs) %>% 
  left_join(precip1, by = c("day", "hour")) %>% 
  left_join(reorder, by = "ID") #%>% 
  #filter(mins.x == 0)
test2 <- test %>% 
  select(datetime.x, binary, hourly_precip, Sequence)
sec2 <- with(test2, train_sec(Sequence, hourly_precip))
...
```{r}
compare_two_sequences <- function(seqA, seqB,
                                  sensor_names = NULL,
                                  methods = c("precedence","kendall","spearman","lcs","edit"),
                                  lcs_normalize = c("min","max","avg")) {
  # seqA, seqB: integer/numeric/character vectors (ordered)
  # sensor_names: optional vector of all known sensors (used to index matrices), otherwise union(seqA, seqB)
  # methods: which measures to compute
  # lcs_normalize: how to normalize LCS length ("min" = /min(lenA,lenB), "max" = /max(...), "avg"= /((lenA+lenB)/2))
  
  methods <- match.arg(methods, several.ok = TRUE)
  lcs_normalize <- match.arg(lcs_normalize)
  
  # Basic checks
  if (!is.vector(seqA) || !is.vector(seqB)) stop("seqA and seqB must be vectors.")
  seqA <- as.character(seqA)
  seqB <- as.character(seqB)
  lenA <- length(seqA); lenB <- length(seqB)
  
  if (is.null(sensor_names)) {
    all_sensors <- sort(unique(c(seqA, seqB)))
  } else {
    all_sensors <- sensor_names
  }
  
  # Helper: precedence indicator matrix for a single sequence
  # returns a named matrix M with entries: 1 if i before j, 0 if i after j, NA if either missing or same
  make_precedence_matrix <- function(seq, all_sensors) {
    n <- length(all_sensors)
    M <- matrix(NA, nrow = n, ncol = n, dimnames = list(all_sensors, all_sensors))
    pos <- match(all_sensors, seq)   # positions (NA if not present)
    for (i in seq_along(all_sensors)) {
      for (j in seq_along(all_sensors)) {
        if (i == j) next
        pi <- pos[i]; pj <- pos[j]
        if (is.na(pi) || is.na(pj)) {
          M[i,j] <- NA
        } else {
          M[i,j] <- ifelse(pi < pj, 1L, 0L)
        }
      }
    }
    return(M)
  ...
  # 1) Precedence agreement (fraction of pairwise orders that agree)
  results <- list()
  if ("precedence" %in% methods) {
    MA <- make_precedence_matrix(seqA, all_sensors)
    MB <- make_precedence_matrix(seqB, all_sensors)
    
    # Consider only pairs where both are non-NA in both matrices
    valid_pairs <- which(!is.na(MA) & !is.na(MB), arr.ind = TRUE)
    # exclude diagonal pairs (same sensor)
    valid_pairs <- valid_pairs[valid_pairs[,1] != valid_pairs[,2], , drop = FALSE]
    n_pairs <- nrow(valid_pairs)
    if (n_pairs == 0) {
      prec_agreement <- NA_real_
    } else {
      agree <- sum(MA[valid_pairs] == MB[valid_pairs])
      prec_agreement <- agree / n_pairs
    }
    results$precedence <- list(agreement = prec_agreement,
                               n_pairs_compared = n_pairs,
                               matrixA = MA,
                               matrixB = MB)
  }
  
  # 2) Kendall's tau on common sensors
  if ("kendall" %in% methods) {
    common <- intersect(seqA, seqB)
    if (length(common) < 2) {
      kendall_tau <- NA_real_
    } else {
      # Build ranks based on order in each sequence (1..k)
      rankA <- match(common, seqA)   # positions in A for common sensors
      rankB <- match(common, seqB)
      # Use cor(method="kendall") which computes Kendall's tau
      kendall_tau <- suppressWarnings(cor(rankA, rankB, method = "kendall", use = "complete.obs"))
    }
    results$kendall <- list(tau = kendall_tau, n_common = length(common), common_sensors = common)
  }
  
  # 3) Spearman rank correlation on common sensors
  if ("spearman" %in% methods) {
    common <- intersect(seqA, seqB)
    if (length(common) < 2) {
      spearman_rho <- NA_real_
    } else {
      rankA <- match(common, seqA)
      rankB <- match(common, seqB)
      spearman_rho <- suppressWarnings(cor(rankA, rankB, method = "spearman", use = "complete.obs"))
    }
    results$spearman <- list(rho = spearman_rho, n_common = length(common))
  }
  
  # 4) LCS (Longest Common Subsequence) normalized
  if ("lcs" %in% methods) {
    # Implement LCS length dynamic programming for two sequences
    lcs_length <- function(x, y) {
      ...
      for (i in seq_len(m)) {
        xi <- x[i]
        for (j in seq_len(n)) {
          if (xi == y[j]) dp[i+1, j+1] <- dp[i, j] + 1L
          else dp[i+1, j+1] <- max(dp[i+1, j], dp[i, j+1])
        }
      }
      dp[m+1, n+1]
    }
    lcs_len <- lcs_length(seqA, seqB)
    denom <- switch(lcs_normalize,
                    min = min(lenA, lenB),
                    max = max(lenA, lenB),
                    avg = (lenA + lenB) / 2)
    if (denom == 0) lcs_norm <- NA_real_ else lcs_norm <- as.numeric(lcs_len) / denom
    results$lcs <- list(length = lcs_len, normalized = lcs_norm,
                        denom_method = lcs_normalize)
  }
  
  # 5) Edit distance (OSA / Levenshtein) on stringified sequences using stringdist if available,
  #    otherwise fall back to simple Levenshtein implementation for short sequences
  if ("edit" %in% methods) {
    # prefer stringdist package if installed
    if (requireNamespace("stringdist", quietly = TRUE)) {
      sA <- paste(seqA, collapse = "\u00FF")  # use a separator unlikely to appear
      sB <- paste(seqB, collapse = "\u00FF")
      ed <- stringdist::stringdist(sA, sB, method = "osa")
      # normalize by max length of token count (or by max(lenA, lenB))
      ed_norm <- ed / max(lenA, lenB)
      results$edit <- list(distance = as.numeric(ed), normalized = as.numeric(ed_norm), method = "osa (stringdist)")
    } else {
      # fallback: compute edit distance on tokens (dynamic programming)
      ed_tokens <- function(x, y) {
        m <- length(x); n <- length(y)
        D <- matrix(0, nrow = m+1, ncol = n+1)
        D[ ,1] <- 0:nrow(D[ ,1]) # WRONG default, fix below
      }
      # simpler fallback: use LCS to approximate (ed ~ lenA + lenB - 2*lcs)
      if (!exists("lcs_len")) {
        # compute lcs if not already
        lcs_length_local <- function(x,y){
          m <- length(x); n <- length(y)
          if (m==0||n==0) return(0L)
          dp <- matrix(0L, m+1, n+1)
          for (i in seq_len(m)) for (j in seq_len(n)) {
            if (x[i]==y[j]) dp[i+1,j+1] <- dp[i,j]+1L else dp[i+1,j+1] <- max(dp[i+1,j], dp[i,j+1])
          }
          dp[m+1,n+1]
        }
        lcs_len <- lcs_length_local(seqA, seqB)
      }
      ed_approx <- lenA + lenB - 2 * lcs_len
      ed_norm <- ed_approx / max(lenA, lenB)
      results$edit <- list(distance_approx = ed_approx, normalized_approx = ed_norm,
                           method = "approx_from_lcs (no stringdist available)")
    }
  }
...
test1 <- compare_two_sequences(W3_pk_seq$ID, w3_twi_sequence$ID)
test1$edit
test2 <- compare_two_sequences(W3_pk_seq$ID, best_bfs_w3$ID)
test2$edit

compare_all_sequences <- function(seq_list,
                                  methods = c("precedence","kendall","spearman","lcs","edit"),
                                  lcs_normalize = "avg",
                                  make_plots = TRUE) {
  
  if (!is.list(seq_list)) stop("seq_list must be a list of sequences.")
  n <- length(seq_list)
  if (n < 2) stop("Need at least 2 sequences.")
  
  seq_names <- names(seq_list)
  if (is.null(seq_names)) seq_names <- paste0("Seq", seq_along(seq_list))
  
  # Storage for long-format results
  results <- list()
  ...
  for (i in 1:(n-1)) {
    for (j in (i+1):n) {
      ...
      res <- compare_two_sequences(sA, sB,
                                   methods = methods,
                                   lcs_normalize = lcs_normalize)
      
      row <- list(
        seqA = nameA,
        seqB = nameB,
        precedence_agreement = if ("precedence" %in% methods) res$precedence$agreement else NA,
        kendall_tau           = if ("kendall"    %in% methods) res$kendall$tau else NA,
        spearman_rho          = if ("spearman"   %in% methods) res$spearman$rho else NA,
        lcs_normalized        = if ("lcs"        %in% methods) res$lcs$normalized else NA,
        edit_normalized       = if ("edit"       %in% methods)
                                  if (!is.null(res$edit$normalized)) res$edit$normalized else
                                    res$edit$normalized_approx else NA
      )
  ...
  results_df <- type.convert(results_df, as.is = TRUE)
  ...
  if (make_plots) {
    if (!requireNamespace("ggplot2", quietly = TRUE)) {
      ...
      # Turn pairwise list into symmetric matrices for each metric
      make_matrix <- function(metric) {
        M <- matrix(NA, nrow = n, ncol = n, dimnames = list(seq_names, seq_names))
        diag(M) <- 1
        for (k in seq_len(nrow(results_df))) {
          a <- results_df$seqA[k]
          b <- results_df$seqB[k]
          val <- results_df[[metric]][k]
          M[a,b] <- val
          M[b,a] <- val
        }
        M
      ...
      metric_names <- c("precedence_agreement","kendall_tau","spearman_rho",
                        "lcs_normalized","edit_normalized")
      
      plot_list <- list()
      
      for (metric in metric_names) {
        if (!metric %in% names(results_df)) next
        
        M <- make_matrix(metric)
        df_plot <- as.data.frame(as.table(M))
        names(df_plot) <- c("Seq1","Seq2","Value")
        
        p <- ggplot(df_plot, aes(x = Seq1, y = Seq2, fill = Value)) +
          geom_tile(color = "white") +
          scale_fill_gradient(low = "white", high = "steelblue", na.value = "grey80") +
          labs(title = paste("Similarity matrix:", metric),
               fill = metric) +
          theme_minimal() +
...
```{r W3-chains-plot}
# Use pk as the basis for color scheme
# renumber so that 1 is the most persistent
renum_w3_pk <- W3_pk_seq %>% 
  mutate(method = "Flow Persistence", x = 1) %>% 
  filter(ID %in% W3_IDs) %>% 
  mutate(sequence = seq(1, 26, 1)) %>% 
  mutate(color_ID = sequence)
# define column for color
color_key <- renum_w3_pk %>% 
  select(ID, sequence) %>% 
  dplyr::rename(color_ID = sequence)
  
# renum_w3_pk <- renum_w3_pk %>% left_join(color_key, by = "ID") %>% 
#   mutate(color_ID = abs(color_ID-26)+1)

renum_w3_twi <- w3_twi_sequence %>% 
  mutate(method = "TWI", x = 3) %>% 
  filter(ID %in% W3_IDs) %>% 
  mutate(sequence = seq(1, 26, 1)) %>% 
  left_join(color_key, by = "ID")

renum_w3_bfs <- best_bfs_w3 %>% 
  mutate(method = "BFS", x = 2) %>% 
  left_join(color_key, by = "ID")

renum_w3_event <- avg_event_w3 %>% 
  mutate(method = "Event", x = 4) %>% 
  left_join(color_key, by = "ID")
...
w3_chains <- 
  ...
  mutate(text_ID = abs(color_ID-26)+1) %>% 
  ggplot()+
  geom_point(aes(x = method, y = sequence, color = color_ID), size = 5)+
  geom_text(aes(x = method, y = sequence, 
                label = text_ID), color = "white", size = 2)+
  scale_x_discrete(labels = function(method) str_wrap(method, width = 10))+
  scale_y_continuous(
    trans = "reverse",
    breaks = c(0, 20),  # Positions for the labels
    labels = c("Wets last,\ndries first", "Wets first,\ndries last")
  ) +
  ...
  theme(axis.ticks.y = element_blank(), # Removes y-axis tick marks
    ...
    axis.text.y = element_text(angle = 90, hjust = 1, vjust = 0.5))    # Removes y-axis line)
```
```{r FB-chains-plot}
# Use pk as the basis for color scheme
# renumber so that 1 is the most persistent
renum_fb_pk <- FB_pk_seq %>% 
  mutate(method = "Flow Persistence", x = 1) %>% 
  filter(ID %in% FB_IDs) %>% 
  #mutate(sequence = seq(24, 1, -1))
  mutate(sequence = seq(1, 24, 1)) %>% 
  mutate(color_ID = sequence)
# define column for color
color_keyFB <- renum_fb_pk %>% 
  select(ID, sequence) %>% 
  dplyr::rename(color_ID = sequence)
  
# renum_fb_pk <- renum_fb_pk %>% left_join(color_keyFB, by = "ID") %>% 
#   mutate(color_ID = abs(color_ID-24)+1)

renum_fb_twi <- fb_twi_sequence %>% 
  mutate(method = "TWI", x = 3) %>% 
  filter(ID %in% FB_IDs) %>% 
  mutate(sequence = seq(1, 24, 1)) %>% 
  left_join(color_keyFB, by = "ID")

renum_fb_bfs <- best_bfs_fb %>% 
  mutate(method = "BFS", x = 2) %>% 
  left_join(color_keyFB, by = "ID")

renum_fb_event <- avg_event_fb %>% 
  mutate(method = "Event", x = 4) %>% 
  left_join(color_keyFB, by = "ID")
...
rbind(
  ...
  mutate(#method = str_wrap(method, 10),
         method = fct_relevel(method, c("Flow Persistence",
                                "TWI",
                                "BFS",
                                "Event"
                                ))
         ) %>% 
  mutate(text_ID = abs(color_ID-24)+1) %>% 
  ggplot()+
  geom_point(aes(x = method, y = sequence, color = color_ID), size = 5)+
  geom_text(aes(x = method, y = sequence, 
                label = text_ID), color = "white", size = 2)+
  ...
  scale_y_continuous(
    trans = "reverse",
    breaks = c(0, 19),  # Positions for the labels
    labels = c("Wets last,\ndries first", "Wets first,\ndries last")
  ) +
  scale_color_viridis_c(guide = "none")+
  theme_classic()+
...
renum_zz_pk <- ZZ_pk_seq %>% 
  ...
  mutate(color_ID = sequence)
# define column for color
color_keyZZ <- renum_zz_pk %>% 
  select(ID, sequence) %>% 
  dplyr::rename(color_ID = sequence)
  
# renum_zz_pk <- renum_zz_pk %>% left_join(color_keyZZ, by = "ID") %>% 
#   mutate(color_ID = abs(color_ID-24)+1)

renum_zz_twi <- zz_twi_sequence %>% 
  mutate(method = "TWI", x = 3) %>% 
  filter(ID %in% ZZ_IDs) %>% 
  mutate(sequence = seq(1, 24, 1)) %>% 
  left_join(color_keyZZ, by = "ID")

renum_zz_bfs <- best_bfs_zz %>% 
  mutate(method = "BFS", x = 2) %>% 
  left_join(color_keyZZ, by = "ID")

renum_zz_event <- avg_event_zz %>% 
  mutate(method = "Event", x = 4) %>% 
  left_join(color_keyZZ, by = "ID")
...
Add maps of each watershed, where points are labeled colored to match these? Maybe just numbered?
```{r maps}
#### W3 ####
# based on figure 1 map
w3_stic_locs_renum <- w3_stic_locs_r %>% 
  left_join(renum_w3_pk) %>% 
  filter(ID %in% W3_IDs)
...
w3_numb_map <- ggplot()+
  ...
  geom_sf(data = w3_outline, fill = NA, color = "#FFD166", alpha = 0.3, lwd = 2)+
  geom_sf(data = w3_net, colour = "#9AC0CD", lwd = 1) +
  geom_sf(data = w3_stic_locs_renum, aes(fill = abs(color_ID-26)+1), size = 4, pch = 21) +
  geom_sf_text(data = w3_stic_locs_renum, aes(label = color_ID), size = 2, color = "white")+
  scale_fill_viridis_c()+
  theme(legend.position = "none")

#### FB ####
fb_stic_locs_renum <- lcc_fb %>% 
  left_join(renum_fb_pk) %>% 
  filter(ID %in% FB_IDs)
...
fb_numb_map <- 
  ...
  geom_sf(data = terra::spin(fb_outline, 90, 279933.5, 4869173), 
          fill = NA, color = "#397367", alpha = 0.3, lwd = 2) +
  geom_sf(data = terra::spin(fb_net, 90, 279933.5, 4869173),
          colour = "darkslategray3", lwd = 2) +
  geom_sf(data = terra::spin(fb_stic_locs_renum, 90, 279933.5, 4869173),
           aes(fill = abs(color_ID-24)+1), size = 4, pch = 21) +
  geom_sf_text(data = terra::spin(fb_stic_locs_renum, 90, 279933.5, 4869173), aes(label = color_ID), size = 2, color = "white")+
  scale_fill_viridis_c()+
  theme(rect = element_rect(fill = "transparent", color = NA))+
  theme(legend.position = "none")

##### ZZ ####
zz_stic_locs_renum <- lcc %>% 
  left_join(renum_zz_pk) %>% 
  filter(ID %in% ZZ_IDs)
...
zz_numb_map <- ggplot()+
  ...
  geom_sf(data = zz_outline, fill = NA, color = "#7E6B8F", alpha = 0.3, lwd = 2) +
  geom_sf(data = zz_net, colour = "darkslategray3", lwd = 1) +
  geom_sf(data = zz_stic_locs_renum, aes(fill = abs(color_ID-24)+1), size = 4, pch = 21) +
  geom_sf_text(data = zz_stic_locs_renum, aes(label = color_ID), size = 2, color = "white")+
  scale_fill_viridis_c() +
  theme(legend.position = "none")
  # scale_fill_continuous(high = "#27408B", low = "white",
  #                       labels = c("0", "0.5", "1"),
...
ggsave("chains_combined.pdf", chains_combined, width = 2, height = 10)

maps_combined <- (w3_numb_map / fb_numb_map / zz_numb_map) + plot_layout(heights = 1)
ggsave("maps_combined.pdf", maps_combined, width = 8, height = 10)

```

## Figure 5: Hydrograph, colored by which sequence works the best
...
df_winners <- 
  df_long_w3 %>%
  # Self-join within each datetime to compare each model to all others
  inner_join(df_long_w3, 
             by = "datetime",
             suffix = c("_this", "_other")) %>%
  
  # Only compare a model to other models
  filter(model_this != model_other) %>%
  
  group_by(datetime, model_this, accuracy_this) %>%
  summarise(
    # max accuracy of other models at that timestamp
    max_other = max(accuracy_other, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  
  mutate(
    is_winner = accuracy_this >= max_other + threshold
  ) %>%
  
  # Keep only winner rows
  filter(is_winner) %>%
  
  # Rename columns back to intuitive names
  dplyr::rename(
    model = model_this,
    accuracy = accuracy_this
  ) %>%
  # Join discharge
  inner_join(w3_discharge_bind, by = "datetime") %>% 
  mutate(model = case_when(model == "bfs_accuracy" ~ "BFS",
                          model == "pk_accuracy" ~ "Proportion of Time Flowing",
                          model == "event_accuracy" ~ "Event based sequence",
                          model == "twi_accuracy" ~ "Topographic Wetness Index")) %>% 
mutate(model = fct_relevel(model, c("Proportion of Time Flowing",
...
ggplot() +
  # Full discharge line for background context
  geom_line(
    data = w3_discharge_bind,
    aes(x = datetime, y = Q_mm_day),
    linewidth = 0.4,
    color = "grey70"
  ) +
  
  # Winner points (faceted)
  geom_point(
    data = df_winners,
    aes(x = datetime, y = Q_mm_day, color = model),
    size = 1
  ) +
  ...
  scale_color_manual(name = "Sequence",
                     values = c(
    "Proportion of Time Flowing"    = "#7ebdc2",
  ...
  labs(
    title = paste0("Times when each model is ≥ ", threshold*100,
                   "% more accurate than all other models (W3)"),
    y = "Discharge (mm/day)",
    x = ""
  ) +
  # This seems to not be compatible anymore?
  # scale_x_break(
  #   breaks = c(ymd_hms("2023-11-12 00:00:00"),
...
```{r W3-updated-AGU}
# Adjustable threshold for how much better a model must be
threshold <- 0.05   # i.e., 5% better than all others
...
df_winners <- 
  ...
  # Keep only rows where a model wins by the threshold
  filter(is_winner) %>%
  ...
  # Model level order for faceting
  ...
  filter(datetime >= ymd_hms("2024-05-20 16:30:00 UTC"))


min_accuracy  <- 0.90  # must exceed 0.60 to "work well"

df_discharge_classified <- df_long_w3 %>%
  group_by(datetime) %>%
  summarise(all_below = all(accuracy < min_accuracy), .groups = "drop") %>%
  inner_join(w3_discharge_bind, by = "datetime") %>% 
  mutate(all_below = case_when(all_below == "FALSE" ~ "Accuracy of all models >= 80%",
                          all_below == "TRUE" ~ "Accuracy of all models < 80%")) 
# %>% 
#   filter(datetime >= ymd_hms("2024-05-20 16:30:00 UTC"))
...
ggplot() +
  ...
  # 🔴 Background: discharge with color based on performance
  geom_line(data = df_discharge_classified,
            aes(x = datetime, y = Q_mm_day, group = 1),
            size = 0.3, color = "black") +
  
  # 🟢 Good model winners (points)
  geom_point(data = df_winners,
             aes(x = datetime, y = Q_mm_day, color = model),
             size = 1) +
  ...
  # scale_x_break(c(
  #   ymd_hms("2024-06-03 00:00:00"),
  #   ymd_hms("2024-07-05 00:00:00")
  # )) +
  
  scale_x_datetime(
  date_labels = "%b %d",        # show month and day: "Jul 03"
  date_breaks = "5 days",       # major tick every 5 days
  date_minor_breaks = "1 day",  # (optional) minor ticks every day
  timezone = "UTC"
) +
  
  # Manual color scale: FALSE = grey, TRUE = black
  scale_color_manual(
    name = "",
    values = c(
    # discharge line colors
    "Accuracy of all models >= 80%" = "grey70",
    "Accuracy of all models < 80%"  = "black",
    # model colors
    "Flow persistence" = "#d68c45",
    "Topographic Wetness Index"  = "#247BA0",
    "BFS"                        = "#2c6e49",
    "Event based sequence"       = "maroon"
  )) +
  
  labs(
    title = paste0("Times when models are ≥ ", threshold*100, "% more accurate (W3)"),
    y = "Discharge (mm/day)",
    x = ""
  )+
...
ggplot() +
  ...
  geom_point(
    data = df_winners_fb,
    aes(x = datetime, y = roll_mean, color = model),
    size = 1
  ) +
  ...
  labs(
    title = paste0("Times when each model is ≥ ", threshold*100,
                   "% more accurate than all other models (FB)"),
    y = "Stage (cm)",
    x = ""
  ) +
  
  # scale_x_break(c(
  #   ymd_hms("2023-10-06 00:00:00 UTC"),
  #   ymd_hms("2024-06-12 17:00:00 UTC")
  # )) +
  ...
  lims(x = c(min(FBall_events$datetime), ymd_hms("2024-7-21 00:00:00 UTC")))+
...
df_winners_zz <- 
  ...
  # Join discharge
  inner_join(ZZall_events, by = "datetime") %>% 
  mutate(model = case_when(model == "bfs_accuracy" ~ "BFS",
                          model == "pk_accuracy" ~ "Proportion of Time Flowing",
                          model == "event_accuracy" ~ "Event based sequence",
                          model == "twi_accuracy" ~ "Topographic Wetness Index")) %>% 
...
ggplot() +
  ...
  labs(
    title = paste0("Times when each model is ≥ ", threshold*100,
                   "% more accurate than all other models (ZZ)"),
    y = "Stage (cm)",
    x = ""
  ) +
  ...
  lims(x = c(min(ZZall_events$datetime), ymd_hms("2024-7-21 00:00:00 UTC")))+
...
Make them again, but a more finalized format
```{r clean-combine}
#### W3 ####
w3_plot7 <- ggplot() +
  # Full discharge line for background context
...
ggsave("figure7_unedited.pdf", exported, scale = 3)

```

Because ggbreak no longer works, and I have illustrator, time to make a nicer version of this plot.
```{r clean-combine}
#### W3 ####
w3_plot7 <- ggplot() +
  # Full discharge line for background context
...
And maybe instead of hydrograph, just show barplot with the number of timesteps where a model was more accurate than all other models, with separate bars for different components of the hydrograph?
```{r W3-updated-AGU}
# Adjustable threshold for how much better a model must be
threshold <- 0.05   # i.e., 5% better than all others
...
min_accuracy  <- 0.80  # must exceed 0.60 to "work well"
...
df_discharge_classified %>% 
  ggplot()+
  geom_bar(aes(fill = x = event_type))
...
## Figure 6: Maps of where sequences work in space
...
ID_acc_w3 <- comparison_w3 %>%
  ...
  tidyr::pivot_longer(cols = c(pk_accuracy, 
                      ...
                      names_to = "model", values_to = "accuracy") %>%
  mutate(
    model = case_when(model == "bfs_accuracy" ~ "BFS",
                      model == "pk_accuracy" ~ "Proportion of Time Flowing",
                      model == "event_accuracy" ~ "Event based sequence",
                      model == "twi_accuracy" ~ "Topographic Wetness Index")) %>% 
mutate(model = fct_relevel(model, c("Proportion of Time Flowing",
...
geom_sf(data =  terra::spin(lcc, 180, 277401.8, 4867071) %>% inner_join(ID_acc_zz, by = "ID"),
        aes(fill = accuracy), size = 3, pch = 21) +
       scale_fill_gradientn(colors = c("white","lightgrey","darkgrey", "black"),
                         limits = c(0,1),
...
ID_acc_w3 <- comparison_w3 %>%
  ...
  tidyr::pivot_longer(cols = c(pk_accuracy, 
                      ...
                      names_to = "model", values_to = "accuracy") %>%
  mutate(
    model = case_when(model == "bfs_accuracy" ~ "Breadth First Search",
                      model == "pk_accuracy" ~ "Flow persistence",
                      model == "event_accuracy" ~ "Event based sequence",
                      model == "twi_accuracy" ~ "Topographic Wetness Index")) %>% 
mutate(model = fct_relevel(model, c("Flow persistence",
                                "Topographic Wetness Index",
...
ggplot()+
  ...
  geom_sf(data = w3_outline, fill = NA, color = "#FFD166", alpha = 0.3, lwd = 1)+
  geom_sf(data = w3_net, colour = "grey", lwd = 1) +
  # geom_sf(data = w3_stic_locs_r %>% 
  #         inner_join(ID_acc_w3, by = "ID") %>% 
  #         filter(ID %not_in% include), size = 1, pch = 20) +
geom_sf(data = w3_stic_locs_r %>% 
          inner_join(ID_acc_w3, by = "ID") %>% 
          filter(ID %in% include), 
        aes(fill = accuracy), size = 5, pch = 21) +
  ...
  #scale_shape_manual(values = c(21, 24, 22))+
  theme(rect = element_rect(fill = "transparent", color = NA),
        strip.text.x = element_text(margin = margin(t = 0, r = 0, b = 10, l = 0, unit = "pt")),
...
ggplot()+
  ...
  geom_sf(data = w3_outline, fill = NA, color = "#FFD166", alpha = 0.3, lwd = 1)+
  geom_sf(data = w3_net, colour = "grey", lwd = 1) +
geom_sf_text(data = w3_stic_locs_r, aes(label = ID), size = 2, color = "black")+
  scale_fill_gradientn(colors = c("white","lightgrey","darkgrey", "black"))+
  #scale_shape_manual(values = c(21, 24, 22))+
  theme(rect = element_rect(fill = "transparent", color = NA),
        legend.position = "none",
...
ID_acc <- comparison_w3 %>%
  ...
  summarize(
    ...
    event_accuracy = event_correct / total) %>%
  select(ID, 
         pk_accuracy, twi_accuracy, bfs_accuracy, event_accuracy) %>%
  tidyr::pivot_longer(cols = c(pk_accuracy, twi_accuracy, bfs_accuracy, event_accuracy), names_to = "model", values_to = "accuracy")
...
length_fb <- sum(perim(fb_net))/1000 #km
#determine watershed area
area_fb <- expanse(fb_outline)/1e6 #km^2
length_fb / area_fb
...
length_zz <- sum(perim(zz_net))/1000 #m
#determine watershed area
area_zz <- expanse(zz_outline)/(1000 * 1000) #m^2
length_zz / area_zz
...
rast(slope_output) %>% 
  crop(zz_outline) %>% 
  terra::mask(zz_outline) %>% 
  plot()
#### Using valley wide slope to calculate average slope ####
w3_avg_slope <- rast(slope_output) %>% 
  crop(w3_outline) %>% 
  terra::mask(w3_outline) %>% 
  global("mean", na.rm=TRUE)
fb_avg_slope <- rast(slope_output) %>% 
  crop(fb_outline) %>% 
  terra::mask(fb_outline) %>% 
  global("mean", na.rm=TRUE)
zz_avg_slope <- rast(slope_output) %>% 
  crop(zz_outline) %>% 
  terra::mask(zz_outline) %>% 
  global("mean", na.rm=TRUE)
#### Use valley wide elevation to calculate min and max elevation in m ####
dem_location <- "/Users/johnmorgan/Documents/VT Research/HBTopmodel/HB/1m hydro enforced DEM/dem1m.tif"
w3_elev <- aggregate(rast(dem_location), 10) %>% 
  crop(w3_outline) %>% 
  terra::mask(w3_outline) %>% 
  global(c("min", "max"), na.rm=TRUE)
fb_elev <- aggregate(rast(dem_location), 10) %>% 
  crop(fb_outline) %>% 
  terra::mask(fb_outline) %>% 
  global(c("min", "max"), na.rm=TRUE)
zz_elev <- aggregate(rast(dem_location), 10) %>% 
  crop(zz_outline) %>% 
  terra::mask(zz_outline) %>% 
  global(c("min", "max"), na.rm=TRUE)

#### Sensor Density ####
# Just take the number of sensors in each watershed, and divide by the total maximum distance of flowing network
dist_per_w3 <- sum(perim(w3_net))/length(W3_IDs)
dist_per_fb <- sum(perim(fb_net))/length(FB_IDs)
dist_per_zz <- sum(perim(zz_net))/length(ZZ_IDs)
#### Expansion Coefficient ####
# Calculate expansion coefficient
# figure out how drainage density varies with the number activated
# W3
ripe_w3 <- input_w3 %>% 
  group_by(datetime) %>% 
  summarise(number_flowing = sum(binary)) %>% 
  mutate(flowing_length_km = number_flowing * dist_per_w3/1000,
         dd = flowing_length_km/area_w3) %>% 
  left_join(w3_discharge_bind) %>% 
  ungroup() %>% 
  mutate(log_length = log(flowing_length_km),
         log_Q = log(Q_mm_day)) %>% 
  select(log_length, log_Q)
ripe_w3 <- ripe_w3[!is.infinite(rowSums(ripe_w3)),]


w3_fit <-  lm(log_length ~ log_Q, data=ripe_w3)
summary(w3_fit)
w3_beta <- unname(w3_fit$coefficients[2])

# FB
ripe_fb <- input_fb %>% 
  group_by(datetime) %>% 
  summarise(number_flowing = sum(binary)) %>% 
  mutate(flowing_length_km = number_flowing * dist_per_fb/1000,
         dd = flowing_length_km/area_fb) %>% 
  left_join(fb_discharge_bind) %>% 
  ungroup() %>% 
  mutate(log_length = log(flowing_length_km),
         log_Q = log(Q_mm_day)) %>% 
  select(log_length, log_Q)
ripe_fb <- ripe_fb[!is.infinite(rowSums(ripe_fb)),]


fb_fit <-  lm(log_length ~ log_Q, data=ripe_fb)
summary(fb_fit)
fb_beta <- unname(fb_fit$coefficients[2])

# ZZ
ripe_zz <- input_zz %>% 
  group_by(datetime) %>% 
  summarise(number_flowing = sum(binary)) %>% 
  mutate(flowing_length_km = number_flowing * dist_per_zz/1000,
         dd = flowing_length_km/area_zz) %>% 
  left_join(w3_discharge_bind) %>% 
  ungroup() %>% 
  mutate(log_length = log(flowing_length_km),
         log_Q = log(Q_mm_day)) %>% 
  select(log_length, log_Q)
ripe_zz <- ripe_zz[!is.infinite(rowSums(ripe_zz)),]

# regression to find B
# W3
w3_fit <-  lm(log_length ~ log_Q, data=ripe_w3)
summary(w3_fit)
w3_beta <- unname(w3_fit$coefficients[2])
w3_beta_r2 <- unname(summary(w3_fit)$r.squared)

#### Final Table ####
topo <- data.frame("Watershed" = c("W3", "FB", "ZZ"),
                   "Drainage Area (ha)" = c(area_w3*100, area_fb*100, area_zz*100),
                   "Maximum Drainage Density m/m^2" = signif(c(length_w3 / area_w3,
                                                        length_fb / area_fb,
                                                        length_zz / area_zz), 2),
                   "Slope" = c(w3_avg_slope$mean, fb_avg_slope$mean, zz_avg_slope$mean),
                   "Elevation_lower" = signif(c(w3_elev$min, fb_elev$min, zz_elev$min),2),
                   "Elevation_upper" = signif(c(w3_elev$max, fb_elev$max, zz_elev$max),2),
                   "Expansion Coefficient (m)" = c(signif(w3_beta, 3),
                                                   "fb_beta",
                                                   "zz_beta"),
                   "Expansion Coefficient R^2" = c(signif(w3_beta_r2,2),
                                                          "XXXX",
                                                          "XXXX"),
                   "Approximate Spacing (m)" = c(100, 50,  75))

names(topo) <- c("Watershed",
                 "Drainage Area (ha)",
                 "Maximum Drainage Density (m/m^2)",
                   "Mean Slope (°)",
                   "Elevation Min (m)",
                   "Elevation Max (m)",
                   "Expansion Coefficient",
                 "Expansion Coefficient R^2",
                   "Approximate Sensor Spacing (m)"
)
nice_table(
  topo
  )
flextable::save_as_image(nice_table(
  topo, width = 0.5
  ), path = "table.png")
...
```{r accuracy-W3}
w3_acc <- rbind(
      produce_metrics(W3_pk_seq, "W3", "Flow persistence"),
      produce_metrics(w3_twi_sequence, "W3", "Topographic Wetness Index"),
      produce_metrics(best_bfs_w3, "W3", "BFS"),
      produce_metrics(avg_event_w3, "W3", "Event-based")
      ) %>% 
  select(Accuracy, Sensitivity, Specificity, shed, method) %>% 
  dplyr::rename(Watershed = shed,
                Sequence = method)
...
```{r accuracy-FB}
fb_acc <- rbind(
      produce_metrics(FB_pk_seq, "FB", "Flow persistence"),
      produce_metrics(fb_twi_sequence, "FB", "Topographic Wetness Index"),
      produce_metrics(best_bfs_fb, "FB", "BFS"),
      produce_metrics(avg_event_fb, "FB", "Event-based")
      ) %>% 
  select(Accuracy, Sensitivity, Specificity, shed, method) %>% 
  dplyr::rename(Watershed = shed,
                Sequence = method)
...
```{r accuracy-ZZ}
zz_acc <- rbind(
      produce_metrics(ZZ_pk_seq, "ZZ", "Flow persistence"),
      produce_metrics(zz_twi_sequence, "ZZ", "Topographic Wetness Index"),
      produce_metrics(best_bfs_zz, "ZZ", "BFS"),
      produce_metrics(avg_event_zz, "ZZ", "Event-based")
      )%>% 
  select(Accuracy, Sensitivity, Specificity, shed, method) %>% 
  dplyr::rename(Watershed = shed,
                Sequence = method)
...
nice_table(bind_rows(w3_acc, fb_acc, zz_acc), spacing = 1) %>% 
