...
FB_air <- 
  read_csv("./PressureTransducers_11_14_23/FB_air.csv", skip = 1) %>% 
  select(2:3) %>% 
  rename(DATETIME = 1,
         pressure_psi_air = 2) %>% 
  mutate(DATETIME = mdy_hms(DATETIME),
         pressure_kpa_air = 6.89476 * pressure_psi_air)
...
FB_water <- read_csv("./PressureTransducers_11_14_23/FB_water.csv", skip = 1) %>% 
  ...
  mutate(DATETIME = mdy_hms(DATETIME),
         pressure_kpa_water = 6.89476 * pressure_psi_water)

#HB headquarters barometric air pressure
headquarters <- read_csv("./HB/HBEF_Barometric_pressure.csv") %>% 
  rename("DATETIME" = TIMESTAMP,
         "pressure_kpa_headquarters" = BP1_kPa) #%>% 
  mutate(pressure_kpa_headquarters = pressure_kpa_headquarters - diff_fb)

#correct headquarters elevation
head_elev <- 254 * 3.28084#m, converted to ft
fb_elev <- 551 * 3.28084#m, converted to ft
zz_elev <- 629 * 3.28084 #m, converted to ft

#decrease pressure by 0.00364 kPa per foot of elevation
diff_fb <- (fb_elev - head_elev) * 0.00364
...
#FB_stage <- 
  FB_water %>% 
  left_join(headquarters, by = "DATETIME") %>% 
    left_join(FB_air, by = "DATETIME") %>% 
    select(-pressure_psi_water, -pressure_psi_air) %>% 
    pivot_longer(-DATETIME) %>% 
  #mutate(diff_kpa = pressure_kpa_water - pressure_kpa_air) %>% 
...
```{r zz-stage-prep}
ZZ_air <- 
  read_csv("./PressureTransducers_11_14_23/ZZ_air.csv", skip = 1) %>% 
  select(2:3) %>% 
  rename(DATETIME = 1,
         pressure_psi_air = 2) %>% 
  #mutate(DATETIME = mdy_hms(DATETIME))%>% 
...
ZZ_water %>% 
  left_join(headquarters, by = "DATETIME") %>% 
    ...
    select(-pressure_psi_water, -pressure_psi_air) %>% 
  rename("Air at stream" = pressure_kpa_air,
         "Water" = pressure_kpa_water,
         "Air at headquarters"= pressure_kpa_headquarters) %>% 
    pivot_longer(-DATETIME) %>% 
  #mutate(diff_kpa = pressure_kpa_water - pressure_kpa_air) %>% 
    ggplot(aes(x = DATETIME, y = value, color = name))+
    geom_line()+
  labs(title = "ZZ barometric pressures",
       x = "",
       y = "Pressure (kPa)")
