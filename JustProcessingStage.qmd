...
processed_fb_stage25 <- FB_water25 %>% 
  ...
  mutate(stage_cm = ((diff_psi*1000) / (997 * 9.8)) * 100 * 0.393701) %>% 
    filter(DATETIME >= ymd_hms(max(processed_fb_stage_24$DATETIME))) %>% 
  mutate(minutes = minute(DATETIME)) %>% 
  filter(minutes %in% c(0, 30)) %>% 
...
dilutionsFB <- q_combined %>% filter(shed == "FB") %>% 
...
#plot(log(dilutions$roll_mean), log(dilutions$discharge))

# fitting linear equation
fb_stage_Q_model <- lm(dilutions$discharge ~ dilutions$roll_mean)

#try exponential model
dilutionsFB_mod <- dilutionsFB[-3,]
model_exp <- lm(log(dilutionsFB_mod$discharge)~ log(dilutionsFB_mod$roll_mean))
summary(model_exp)
