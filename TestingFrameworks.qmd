...
##Fitting my own logistic regression model based on topography
```{r}
...
#get coordinates for Watershed 3 locations
locs <- data_23 %>% 
  filter(wshed == "W3") %>% 
  select(ID, lat, long) %>% 
  unique()
...
# 12/4/24- cementing this analysis, saving shapefile with points
#note that it is in the wrong projection
...
##make this plot again but with improved geospatial grounding
```
